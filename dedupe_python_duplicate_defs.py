...
def find_units(lines):
    """Return list of (kind, name, start, end). `start` includes any
    directly-preceding, contiguous decorator line(s) (2026-09-10: a duplicate
    `@decorator`-prefixed def/class used to leave its decorator orphaned when
    dropped, since the gap-preservation walk in dedupe() always keeps content
    between units regardless of keep/drop, and the decorator sat in that gap
    rather than inside the unit it modifies - producing invalid Python).
    """
    raw_starts = []
    for i, line in enumerate(lines):
        m = DEF_RE.match(line)
        if m:
            raw_starts.append((i, m.group(1), m.group(2)))

    starts = []
    prev_raw_start = -1
    for raw_start, kind, name in raw_starts:
        adj_start = raw_start
