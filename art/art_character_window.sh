...
timeout 2400 "$PY" "$HERE/art_pipeline_run.py" --role dead --passes 3 >>"$OUT" 2>&1 && log "dead ok" || log "dead FAILED"
log "=== DOWNED (new, QA-gated) ==="
timeout 2400 "$PY" "$HERE/art_pipeline_run.py" --role downed --passes 3 >>"$OUT" 2>&1 && log "downed ok" || log "downed FAILED"
