# Grooming proposal — shrike-notify — 2026-09-14 04:00
...
1. Add unit tests for `backend/app/utils/backoff.py` verifying exponential growth and max cap behavior
2. Add unit tests for `backend/app/utils/dedupe.py` ensuring duplicate message removal logic handles edge cases
3. Implement `GET /health` endpoint in `backend/app/main.py` returning service status and version info
