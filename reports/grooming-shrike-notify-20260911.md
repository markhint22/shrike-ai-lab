# Grooming proposal — shrike-notify — 2026-09-11 04:00
...
1. Add unit tests for `backend/app/utils/dedupe.py` covering duplicate removal and order preservation
2. Implement `GET /health` endpoint in `backend/app/main.py` returning service status and version
3. Add integration test for `POST /messages` in `backend/tests/test_core.py` verifying 201 response and persistence
