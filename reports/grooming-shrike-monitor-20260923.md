# Grooming proposal — shrike-monitor — 2026-09-23 04:00
...
1. Add unit tests for `backend/app/utils/percentile.py` covering edge cases (empty list, single value, exact p95/p50 boundaries) and verify with `pytest tests/test_percentile.py`.
2. Add unit tests for `backend/app/utils/backoff.py` verifying exponential growth, max cap enforcement, and jitter application logic.
3. Add unit tests for `backend/app/utils/debounce.py` ensuring state transitions (idle -> pending -> fired) respect the configured delay and reset on new events.
4. Add unit tests for `backend/app/utils/downtime.py` calculating total downtime duration from a list of `CheckResult` objects, handling overlapping or adjacent down periods.
5. Add unit tests for `backend/app/utils/status_streak.py` verifying correct counting of consecutive identical statuses and reset on status change.
