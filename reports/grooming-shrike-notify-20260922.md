# Grooming proposal — shrike-notify — 2026-09-22 04:00
...
1. Add unit tests for `backend/app/utils/quiet_hours.py` verifying that notifications are suppressed during configured quiet windows and allowed outside them, including edge cases for timezone boundaries.
2. Implement a `GET /health` endpoint in `backend/app/main.py` that returns the status of the broker and database connection, ensuring it is excluded from rate limiting and authentication middleware.
3. Add integration tests for `backend/scripts/notify_smoke_check.py` to verify that the script correctly exits with code 0 on successful notification delivery and non-zero on simulated network failures.
4. Refactor `backend/app/services/ratelimit.py` to extract the token bucket state into a separate dataclass, improving testability and reducing coupling between logic and state management.
5. Add tests for `backend/app/utils/truncate.py` ensuring that message bodies are correctly truncated to the configured maximum length without breaking multi-byte character encoding.
