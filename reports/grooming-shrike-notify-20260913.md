# Grooming proposal — shrike-notify — 2026-09-13 04:00
...
1. Add unit tests for `backend/app/utils/quiet_hours.py` covering edge cases where the current time is exactly at the start or end of a quiet window.
2. Implement a `GET /health` endpoint in `backend/app/main.py` that returns service status and database connectivity check results.
3. Add integration tests for `backend/app/delivery.py` verifying that messages are correctly routed to subscribers based on topic filters.
