# Grooming proposal — shrike-monitor — 2026-09-25 04:00
...
1. Add tests for `clamp` in backend/app/utils/clamp.py covering boundary values (min, max, within range) and edge cases (NaN, inf) + verify with pytest
2. Add tests for `percentile` in backend/app/utils/percentile.py covering empty lists, single element, and standard distributions (p50, p95) + verify with pytest
3. Add tests for `severity_from_code` in backend/app/utils/severity_from_code.py mapping HTTP status codes to severity levels (2xx->up, 4xx->degraded, 5xx->down) + verify with pytest
4. Add tests for `status_streak` in backend/app/utils/status_streak.py counting consecutive identical statuses from a list of CheckResults + verify with pytest
5. Add tests for `prune_threshold` in backend/app/utils/prune_threshold.py determining when to prune old results based on count/age + verify with pytest
6. Add tests for `round_sig` in backend/app/utils/round_sig.py rounding floats to significant figures (e.g., 123.456 -> 123.0 for 3 sig figs) + verify with pytest
7. Add a `Store.prune_old(monitor_id, max_age_seconds)` method in backend/app/store.py to remove results older than threshold + tests
8. Add a `Monitor.get_current_status()` helper in backend/app/models.py returning the status of the most recent CheckResult or 'pending' if none + tests
