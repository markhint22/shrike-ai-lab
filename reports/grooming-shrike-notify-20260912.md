# Grooming proposal — shrike-notify — 2026-09-12 04:00
...
1. Add unit tests for `backend/app/utils/dedupe.py` covering duplicate message removal and edge cases (empty list, single item).
2. Add unit tests for `backend/app/utils/coalesce.py` verifying correct merging of consecutive identical messages.
3. Implement a `DELETE /tokens/{token_id}` endpoint in `backend/app/routers/publish.py` to revoke specific tokens, protected by admin scope.
