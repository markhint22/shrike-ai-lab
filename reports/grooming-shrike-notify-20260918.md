# Grooming proposal — shrike-notify — 2026-09-18 04:00
...
1. Add unit tests for `backend/app/utils/dedupe.py` covering exact-match removal, case-insensitive normalization, and empty-input edge cases.
2. Implement a `GET /health` endpoint in `backend/app/main.py` that returns 200 OK with a JSON body containing the current timestamp and service version.
3. Add tests for `backend/app/utils/backoff.py` verifying exponential growth logic, maximum cap enforcement, and jitter randomness bounds.
4. Create a `POST /messages/bulk` endpoint in `backend/app/routers/messages.py` accepting a list of messages, validating each against the schema, and returning a summary of accepted/rejected counts.
5. Add tests for `backend/app/utils/quiet_hours.py` ensuring correct boolean returns for times inside/outside quiet windows and handling of timezone offset edge cases.
