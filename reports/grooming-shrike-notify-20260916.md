# Grooming proposal — shrike-notify — 2026-09-16 04:00
...
1. Add unit tests for `backend/app/utils/dedupe.py` covering duplicate removal logic and edge cases (empty list, single item).
2. Add unit tests for `backend/app/utils/coalesce.py` verifying correct merging of overlapping time windows.
3. Add unit tests for `backend/app/utils/backoff.py` ensuring exponential backoff calculation respects max retry limits.
