# Grooming proposal — shrike-notify — 2026-09-20 04:00
...
2. Implement a `GET /health` endpoint in `backend/app/main.py` that returns the status of the broker connection and database persistence layer, ensuring it is excluded from rate limiting.
3. Add integration tests for `backend/app/retention.py` to verify that messages older than the configured TTL are correctly purged from the broker and that active subscribers are notified of the cleanup.
