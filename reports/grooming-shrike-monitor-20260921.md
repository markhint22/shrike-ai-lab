# Grooming proposal — shrike-monitor — 2026-09-21 04:00
...
1. Add unit tests for `backend/app/utils/percentile.py` covering empty lists, single values, and p95 calculation accuracy.
2. Implement a `Store.get_monitor(monitor_id)` method in `backend/app/store.py` to retrieve a specific monitor object by ID, with corresponding tests.
3. Add integration tests for the `POST /monitors/{id}/heartbeat` endpoint in `backend/tests/test_monitors_api.py` to verify status updates and latency recording.
