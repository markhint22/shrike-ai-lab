# Grooming proposal — shrike-monitor — 2026-09-16 04:00
...
1. Add unit tests for `backend/app/utils/backoff.py` covering exponential growth, max cap, and jitter application to ensure retry logic is deterministic and bounded.
2. Add unit tests for `backend/app/utils/downtime.py` verifying correct calculation of total downtime duration from a list of `CheckResult` objects with mixed statuses.
3. Add unit tests for `backend/app/utils/status_streak.py` to validate the logic for counting consecutive identical statuses (e.g., 3 consecutive 'down' results) and handling status changes.
