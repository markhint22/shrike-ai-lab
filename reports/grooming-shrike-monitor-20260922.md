# Grooming proposal — shrike-monitor — 2026-09-22 04:00
...
1. Add tests for `Store.record_probe` and `Store.latest` in backend/app/store.py to verify trimming logic and most-recent retrieval behavior
2. Implement a `Store.get_history(monitor_id, limit)` method in backend/app/store.py to return a list of the last N results for a specific monitor
3. Add tests for `Store.get_history` in backend/tests/test_store.py covering empty history, exact limit, and exceeding limit scenarios
4. Add a `GET /monitors/{id}/history` endpoint in backend/app/routers/monitors.py that returns the last 10 results using `Store.get_history`
5. Add tests for the new `/monitors/{id}/history` endpoint in backend/tests/test_monitors_api.py verifying correct status codes and response structure
6. Add a `GET /health/detailed` endpoint in backend/app/main.py that returns system uptime, current monitor count, and last check timestamp
7. Add tests for the `/health/detailed` endpoint in backend/tests/test_main_config_verification.py to ensure it returns valid JSON with expected keys
