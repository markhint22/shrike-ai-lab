# Grooming proposal — shrike-notify — 2026-09-19 04:00
...
1. Add unit tests for `TopicStats` aggregation logic in `backend/app/services/topic_stats.py` (verify message counts and error rates per topic)
2. Implement `GET /stats` endpoint in `backend/app/routers/messages.py` to expose aggregated topic statistics, protected by read scope
3. Add integration test for `backend/app/delivery.py` verifying that messages exceeding TTL are dropped before delivery
4. Create `backend/tests/test_quiet_hours.py` to validate `is_in_quiet_hours` logic against timezone-aware datetime inputs
5. Add unit tests for `backend/app/utils/truncate.py` ensuring message bodies are truncated to the configured max length without breaking UTF-8 characters
