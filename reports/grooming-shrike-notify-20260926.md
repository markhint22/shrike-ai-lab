# Grooming proposal — shrike-notify — 2026-09-26 04:00
...
1. Add unit tests for `backend/app/utils/backoff.py` verifying exponential growth and max delay capping (quick win, high value for reliability).
2. Implement `GET /stats` endpoint in `backend/app/routers/subscribe.py` to expose topic statistics using `backend/app/services/topic_stats.py` (high value, fills missing observability gap).
3. Add integration test for `backend/app/utils/quiet_hours.py` ensuring notifications are suppressed during configured quiet windows (quick win, critical for user experience).
