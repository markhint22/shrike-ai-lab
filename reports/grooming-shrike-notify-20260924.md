# Grooming proposal — shrike-notify — 2026-09-24 04:00
...
1. Add unit tests for `backend/app/utils/dedupe.py` covering key generation, collision handling, and the sliding window expiration logic.
2. Implement a `GET /health` endpoint in `backend/app/main.py` that returns 200 OK with service version and basic dependency status (DB/Broker connectivity).
3. Add integration tests for `backend/app/services/ttl.py` verifying that messages exceeding the TTL are correctly purged from the broker store.
4. Create a `POST /messages/bulk` endpoint in `backend/app/routers/messages.py` to accept a list of messages, validating each against the schema and returning per-item status codes.
5. Add tests for `backend/app/utils/quiet_hours.py` ensuring that notifications are suppressed during configured quiet windows and allowed outside them.
6. Implement a `DELETE /tokens/{token_id}` endpoint in `backend/app/routers/publish.py` to revoke a specific token, protected by admin scope.
7. Add unit tests for `backend/app/utils/backoff.py` verifying the exponential backoff calculation and jitter application for retry logic.
8. Create a `GET /stats/topics` endpoint in `backend/app/routers/subscribe.py` (or new router) to expose topic statistics using `backend/app/services/topic_stats.py`.
