# Grooming proposal — shrike-monitor — 2026-09-14 04:00
...
1. Add a `Store.get_monitor` method in backend/app/store.py to retrieve a single monitor by ID, returning None if not found, and add corresponding tests in backend/tests/test_core.py.
2. Implement a `calculate_uptime_percentage` helper in backend/app/utils/uptime.py that computes uptime % from a list of CheckResults over a given time window, handling empty lists and all-down scenarios, with tests in backend/tests/test_uptime.py.
3. Add a `validate_heartbeat_payload` function in backend/app/services/heartbeat.py to ensure the incoming JSON contains required fields (monitor_id, timestamp) and valid types, raising a specific ValidationError, with tests in backend/tests/test_heartbeat.py.
