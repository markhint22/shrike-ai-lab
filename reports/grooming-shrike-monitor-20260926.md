# Grooming proposal — shrike-monitor — 2026-09-26 04:00
...
1. Add unit tests for `backend/app/utils/percentile.py` covering empty lists, single values, and p50/p95 boundary cases to ensure statistical accuracy.
2. Add unit tests for `backend/app/utils/debounce.py` verifying that repeated calls within the window return cached results and subsequent calls after expiry trigger new execution.
3. Add unit tests for `backend/app/utils/status_streak.py` validating streak counting logic for mixed status sequences (e.g., up/down/up) and edge cases like empty inputs.
