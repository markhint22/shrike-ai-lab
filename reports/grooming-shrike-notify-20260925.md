# Grooming proposal — shrike-notify — 2026-09-25 04:00
...
1. Add unit tests for `backend/app/utils/dedupe.py` verifying that duplicate message hashes are dropped and unique messages pass through, including edge cases with empty input lists.
2. Implement a `GET /health` endpoint in `backend/app/main.py` that returns a 200 status code and a JSON body containing the service version and current timestamp for load balancer checks.
3. Add tests for `backend/app/utils/quiet_hours.py` to verify that notifications are suppressed during configured quiet hours and allowed outside of them, covering timezone-aware datetime comparisons.
4. Create a `POST /messages/bulk` endpoint in `backend/app/routers/messages.py` that accepts a list of message schemas, validates each against the rate limiter, and returns a summary of accepted vs. rejected items.
5. Add integration tests for `backend/app/services/topic_stats.py` to ensure that topic statistics (message count, last seen time) are correctly updated after publishing messages to the broker.
