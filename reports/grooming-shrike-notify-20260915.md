# Grooming proposal — shrike-notify — 2026-09-15 04:00
...
1. Add unit tests for `backend/app/utils/quiet_hours.py` verifying that message delivery is correctly suppressed during configured quiet hours and allowed outside them.
2. Add unit tests for `backend/app/utils/severity.py` ensuring that severity levels are correctly parsed, normalized, and mapped to the expected priority values.
3. Implement a `GET /health` endpoint in `backend/app/main.py` that returns a 200 status with basic service metadata (version, uptime) for load balancer health checks.
