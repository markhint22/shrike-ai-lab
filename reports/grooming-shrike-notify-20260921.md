# Grooming proposal — shrike-notify — 2026-09-21 04:00
...
1. Add unit tests for `backend/app/utils/dedupe.py` covering hash collision handling and window expiration logic.
2. Add unit tests for `backend/app/utils/backoff.py` verifying exponential increase and jitter bounds.
3. Implement `GET /stats/topics` endpoint in `backend/app/routers/publish.py` to expose per-topic message counts using `topic_stats.py`.
4. Add integration test for `backend/app/services/ttl.py` verifying that expired messages are purged from the broker after TTL elapses.
5. Refactor `backend/app/utils/quiet_hours.py` to accept a timezone parameter and add tests for DST transition edge cases.
