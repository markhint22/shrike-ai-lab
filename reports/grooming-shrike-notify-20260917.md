# Grooming proposal — shrike-notify — 2026-09-17 04:00
...
1. Add unit tests for `backend/app/utils/dedupe.py` covering duplicate removal logic and edge cases (empty lists, mixed types) to ensure delivery integrity.
2. Implement a `GET /health` endpoint in `backend/app/main.py` that returns service status and dependency checks (DB, Broker) for monitoring integration.
3. Add integration tests for `backend/app/services/receipt.py` verifying that message acknowledgments are correctly persisted and retrievable by client ID.
