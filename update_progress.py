...
def containment(done_key, item_key):
    """Fraction of the DONE key's tokens present in the item — catches a
    SHORT DONE declaration against a VERBOSE curated item (where symmetric
    token-overlap stays low even though the DONE clearly names the item)."""
    sd, si = set(done_key.split()), set(item_key.split())
    if not sd:
        return 0.0
    return len(sd & si) / len(sd)
...
def main():
    ...
    def classify(spans, key, done_state):
        ...
        # Containment tier (2026-08-28): the DONE's words are (almost) all
        # inside exactly one item — the common short-DONE / verbose-item case.
        contained = []
        for (s, _e) in spans:
            if is_done_item(lines[s]) != done_state:
                continue
            ik = normalize(lines[s])
            if ik:
                contained.append((containment(key, ik), s))
        contained.sort(reverse=True)
        if contained and contained[0][0] >= 0.85 and (
                len(contained) == 1 or contained[0][0] - contained[1][0] >= MARGIN):
            return 'fuzzy', contained[0][1]
