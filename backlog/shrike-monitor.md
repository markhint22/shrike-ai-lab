...
- [ ] [T1] backend/app/utils/humanize.py — Remove the `humanize_bytes` function definition and its associated docstring. VERIFY: `grep -n "def humanize_bytes" backend/app/utils/humanize.py` returns no output. (cat:refactor; multifile:no) [feat:shrike-monitor-20260921-app-utils-humanize-py-s-humanize-bytes-i]
- [ ] [T2] backend/tests/test_utils.py — Delete all test functions starting with `test_humanize_bytes` and any imports specifically for `humanize_bytes`. VERIFY: `grep -n "humanize_bytes" backend/tests/test_utils.py` returns no output. (cat:test; multifile:no) [feat:shrike-monitor-20260921-app-utils-humanize-py-s-humanize-bytes-i]
- [ ] [T3] backend/app/utils/humanize.py — Verify the file still contains `humanize_count` and other remaining utilities if any, or delete the file entirely if it becomes empty. VERIFY: `ls -l backend/app/utils/humanize.py` shows the file exists with content OR `test -f backend/app/utils/humanize.py` fails (if deleted). (cat:refactor; multifile:no) [feat:shrike-monitor-20260921-app-utils-humanize-py-s-humanize-bytes-i]
- [ ] [T4] backend/tests/test_utils.py — Ensure remaining tests in `test_utils.py` still pass after removing `humanize_bytes` tests. VERIFY: `cd backend && .venv/bin/pytest tests/test_utils.py -v` exits with code 0. (cat:test; multifile:no) [feat:shrike-monitor-20260921-app-utils-humanize-py-s-humanize-bytes-i]
- [ ] [T5] backend/app/utils/__init__.py — Check if `humanize_bytes` is exported in `__all__` or imported explicitly, and remove it if so. VERIFY: `grep -n "humanize_bytes" backend/app/utils/__init__.py` returns no output. (cat:refactor; multifile:no) [feat:shrike-monitor-20260921-app-utils-humanize-py-s-humanize-bytes-i]
...
- [ ] [T1] backend/tests/test_store.py — Add a new test function `test_record_probe_method_removed` that asserts `not hasattr(Store, "record_probe")` to enforce the dead code removal. VERIFY: `cd backend && .venv/bin/pytest tests/test_store.py::test_record_probe_method_removed -v`. (cat:test; multifile:no) [feat:shrike-monitor-20260921-store-record-probe-is-a-dead-convenience]
- [ ] [T2] backend/tests/test_store.py — Delete the existing test function `test_record_probe_records_result` which validates the behavior of the soon-to-be-deleted method. VERIFY: `cd backend && .venv/bin/pytest tests/test_store.py -v`. (cat:test; multifile:no) [feat:shrike-monitor-20260921-store-record-probe-is-a-dead-convenience]
- [ ] [T3] backend/app/store.py — Remove the `record_probe` method definition (lines 139-141) from the `Store` class, ensuring `record` remains intact. VERIFY: `cd backend && .venv/bin/pytest tests/test_store.py -v`. (cat:refactor; multifile:no) [feat:shrike-monitor-20260921-store-record-probe-is-a-dead-convenience]
- [ ] [T4] backend/app/store.py — Verify that `Store.record` method signature and implementation remain unchanged to confirm the live write path is untouched. VERIFY: `cd backend && grep -n "def record" backend/app/store.py`. (cat:python; multifile:no) [feat:shrike-monitor-20260921-store-record-probe-is-a-dead-convenience]
- [ ] [T5] backend/tests/test_store.py — Ensure no other tests in the file reference `record_probe` by running a full test suite pass on the store tests. VERIFY: `cd backend && .venv/bin/pytest tests/test_store.py -v`. (cat:test; multifile:no) [feat:shrike-monitor-20260921-store-record-probe-is-a-dead-convenience]
...
- [ ] [T1] backend/app/utils/backoff.py — Move the string literal `"""Exponential backoff with jitter for retry logic."""` from line 34 to line 1 as the module docstring, removing it from the end of the file. VERIFY: `cd backend && .venv/bin/python -c "import app.utils.backoff as m; assert m.__doc__ == 'Exponential backoff with jitter for retry logic.'"`. (cat:python; multifile:no) [feat:shrike-monitor-20260921-app-utils-backoff-py-has-a-dead-unreacha]
- [ ] [T2] backend/tests/test_backoff.py — Add a test function `test_module_docstring` that asserts `app.utils.backoff.__doc__` is not None and contains "backoff". VERIFY: `cd backend && .venv/bin/pytest tests/test_backoff.py::test_module_docstring -v`. (cat:test; multifile:no) [feat:shrike-monitor-20260921-app-utils-backoff-py-has-a-dead-unreacha]
- [ ] [T3] backend/app/utils/backoff.py — Verify that `calculate_backoff_delay` still imports correctly and executes without syntax errors after the docstring move. VERIFY: `cd backend && .venv/bin/python -c "from app.utils.backoff import calculate_backoff_delay; print('OK')"`. (cat:python; multifile:no) [feat:shrike-monitor-20260921-app-utils-backoff-py-has-a-dead-unreacha]
- [ ] [T4] backend/tests/test_backoff.py — Run the full existing test suite for backoff to ensure no regressions in delay calculation logic. VERIFY: `cd backend && .venv/bin/pytest tests/test_backoff.py -v`. (cat:test; multifile:no) [feat:shrike-monitor-20260921-app-utils-backoff-py-has-a-dead-unreacha]
- [ ] [T5] backend/app/utils/backoff.py — Confirm that the file structure matches sibling conventions by checking that the first line is a docstring and the second line is an import. VERIFY: `cd backend && head -2 app/utils/backoff.py | grep -E '^(\"\"\"|import)'`. (cat:refactor; multifile:no) [feat:shrike-monitor-20260921-app-utils-backoff-py-has-a-dead-unreacha]
