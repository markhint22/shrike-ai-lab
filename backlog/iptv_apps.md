...
- [ ] [T1] iptv-backend/tests/test_stream_analytics_aggregation.py — Verify that the docstring of the surviving `TestEdgeCases` class is preserved and identical to the original intent. VERIFY: `grep -A 2 "class TestEdgeCases" iptv-backend/tests/test_stream_analytics_aggregation.py | head -3` shows the docstring line. (cat:test; multifile:no) [feat:iptv_apps-20260922-test-stream-analytics-aggregation-py-def]
...
- [ ] [T1] iptv-backend/app/services/stream_import_guard.py — Replace module content with a docstring explicitly stating "placeholder", "not wired", and "pending real content-rights decision" followed by `pass` to remove the fake `is_source_licensed` function. VERIFY: `head -n 1 iptv-backend/app/services/stream_import_guard.py | grep -q "placeholder\|not wired\|pending"` AND `python -c "import ast; ast.parse(open('iptv-backend/app/services/stream_import_guard.py').read())"`. (cat:python; multifile:no) [feat:iptv_apps-20260922-stream-import-guard-py-is-an-orphaned-de]
- [ ] [T2] iptv-backend/tests/test_stream_import_guard.py — Rewrite test to import the module and assert that the `__doc__` attribute contains "placeholder", "not wired", or "pending", and that `is_source_licensed` is not defined in the module namespace. VERIFY: `cd iptv-backend && python -m pytest tests/test_stream_import_guard.py -v`. (cat:test; multifile:no) [feat:iptv_apps-20260922-stream-import-guard-py-is-an-orphaned-de]
- [ ] [T3] iptv-backend/app/services/stream_import_guard.py — Ensure no other code in `app/` imports or references `stream_import_guard` or `is_source_licensed` by verifying the module is truly orphaned and safe to leave as a stub. VERIFY: `grep -rn "stream_import_guard\|is_source_licensed" iptv-backend/app --include="*.py" | grep -v "stream_import_guard.py"` returns empty. (cat:refactor; multifile:no) [feat:iptv_apps-20260922-stream-import-guard-py-is-an-orphaned-de]
