...
- [ ] [T1] iptv-backend/app/services/search.py — Extract a pure helper `build_token_filters(term: str)` that returns a list of SQLAlchemy filter expressions using `tokenize()` from `search_query_tokenize`, applying ilike to name/category/group for each token. VERIFY: python -c "from app.services.search import build_token_filters; print(len(build_token_filters('trek star')) == 2)" (cat:python; multifile:no)
- [ ] [T1] iptv-backend/tests/test_search_token_filters.py — Create a new test file that imports `build_token_filters` and asserts it generates correct ilike patterns for multi-word inputs, handling edge cases like empty strings or single words. VERIFY: pytest tests/test_search_token_filters.py -v (cat:test; multifile:no)
- [ ] [T3] iptv-backend/app/services/search.py — Modify `build_candidate_query()` to replace the single raw `f"%{term}%"` ilike with an AND-combination of the filters returned by `build_token_filters(term)`. VERIFY: grep -q "build_token_filters" iptv-backend/app/services/search.py (cat:python; multifile:no)
- [ ] [T2] iptv-backend/tests/test_search.py — Add a test case `test_reordered_multi_word_query` that mocks the database session, calls `search_streams` with "trek star", and asserts the resulting query filters match those for "star trek". VERIFY: pytest tests/test_search.py::test_reordered_multi_word_query -v (cat:test; multifile:no)
- [ ] [T2] iptv-backend/tests/test_search_filters.py — Add a test case verifying that a multi-word query like "trek star" correctly matches a stream named "Star Trek" by checking the SQL expression structure or mock execution. VERIFY: pytest tests/test_search_filters.py -k "reordered" -v (cat:test; multifile:no)
- [ ] [T1] iptv-backend/app/services/search_query_tokenize.py — Add a docstring example to `tokenize()` showing that it splits on whitespace and normalizes case, ensuring the contract is clear for the new consumer. VERIFY: python -c "from app.services.search_query_tokenize import tokenize; assert tokenize('Star Trek') == ['star', 'trek']" (cat:python; multifile:no)
- [ ] [T3] iptv-backend/app/routers/search.py — Ensure `search_streams()` passes the raw query string to `build_candidate_query` without pre-normalization, relying on the service layer to handle tokenization. VERIFY: grep -q "query" iptv-backend/app/routers/search.py && pytest tests/test_search.py -v (cat:endpoint; multifile:no)
