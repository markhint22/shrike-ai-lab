...
- [ ] [T5] iptv-ios/ChickadeeStreamsTV/Views/TVHistoryView.swift — Ensure the `.alert` modifier is attached to the root view hierarchy element (e.g., `ZStack` or `NavigationStack`) to ensure proper presentation context. VERIFY: `grep -A 2 "fetchError" iptv-ios/ChickadeeStreamsTV/Views/TVHistoryView.swift | grep -q "alert"`. (cat:refactor; multifile:yes)
...
- [ ] [T1] iptv-android/app/src/test/java/com/chickadeestreams/iptv/ui/mobile/viewmodels/WatchlistViewModelTest.kt — Create test file with MockK setup for ChickadeeApi and UnconfinedTestDispatcher. VERIFY: `./gradlew :app:testDebugUnitTest --tests "com.chickadeestreams.iptv.ui.mobile.viewmodels.WatchlistViewModelTest"` passes. (cat:test; multifile:no)
- [ ] [T2] iptv-android/app/src/test/java/com/chickadeestreams/iptv/ui/mobile/viewmodels/WatchlistViewModelTest.kt — Add test `load_success_populates_streams` verifying uiState.streams is updated on 200 response. VERIFY: `./gradlew :app:testDebugUnitTest --tests "com.chickadeestreams.iptv.ui.mobile.viewmodels.WatchlistViewModelTest.load_success_populates_streams"` passes. (cat:test; multifile:no)
- [ ] [T2] iptv-android/app/src/test/java/com/chickadeestreams/iptv/ui/mobile/viewmodels/WatchlistViewModelTest.kt — Add test `load_non_2xx_sets_error` verifying uiState.error is set on 404 response. VERIFY: `./gradlew :app:testDebugUnitTest --tests "com.chickadeestreams.iptv.ui.mobile.viewmodels.WatchlistViewModelTest.load_non_2xx_sets_error"` passes. (cat:test; multifile:no)
- [ ] [T2] iptv-android/app/src/test/java/com/chickadeestreams/iptv/ui/mobile/viewmodels/WatchlistViewModelTest.kt — Add test `load_exception_sets_error` verifying uiState.error is set when API throws IOException. VERIFY: `./gradlew :app:testDebugUnitTest --tests "com.chickadeestreams.iptv.ui.mobile.viewmodels.WatchlistViewModelTest.load_exception_sets_error"` passes. (cat:test; multifile:no)
- [ ] [T2] iptv-android/app/src/test/java/com/chickadeestreams/iptv/ui/mobile/viewmodels/WatchlistViewModelTest.kt — Add test `remove_success_updates_ui` verifying item is removed from uiState.streams on 204 response. VERIFY: `./gradlew :app:testDebugUnitTest --tests "com.chickadeestreams.iptv.ui.mobile.viewmodels.WatchlistViewModelTest.remove_success_updates_ui"` passes. (cat:test; multifile:no)
- [ ] [T2] iptv-android/app/src/test/java/com/chickadeestreams/iptv/ui/mobile/viewmodels/WatchlistViewModelTest.kt — Add test `remove_failure_rolls_back` verifying item is restored to uiState.streams on 500 response. VERIFY: `./gradlew :app:testDebugUnitTest --tests "com.chickadeestreams.iptv.ui.mobile.viewmodels.WatchlistViewModelTest.remove_failure_rolls_back"` passes. (cat:test; multifile:no)
...
- [ ] [T1] iptv-backend/app/services/playlist_group_order.py — Remove the duplicate second definition of `order_groups` (lines 47-76) keeping only the first canonical implementation. VERIFY: `grep -c "def order_groups" iptv-backend/app/services/playlist_group_order.py` returns 1. (cat:refactor; multifile:no)
- [ ] [T2] iptv-backend/tests/test_playlist_group_order.py — Remove the duplicate second `TestOrderGroups` class, keeping only the first canonical test suite. VERIFY: `grep -c "class TestOrderGroups" iptv-backend/tests/test_playlist_group_order.py` returns 1. (cat:test; multifile:no)
- [ ] [T3] iptv-backend/app/routers/streams.py — Import `order_groups` from `app.services.playlist_group_order` and apply it to the result of `query.distinct().all()` in `get_categories()` before returning, ensuring categories are sorted. VERIFY: `grep -n "order_groups" iptv-backend/app/routers/streams.py` shows the import and usage within `get_categories`. (cat:endpoint; multifile:no)
- [ ] [T4] iptv-backend/tests/test_streams.py — Add a new test case `test_get_categories_returns_sorted` that mocks the DB query to return unsorted categories and asserts the response is in case-insensitive alphabetical order. VERIFY: `pytest iptv-backend/tests/test_streams.py::test_get_categories_returns_sorted -v` passes. (cat:test; multifile:no)
- [ ] [T5] iptv-backend/tests/test_playlist_group_order.py — Run the existing test suite to ensure no regressions occurred after removing the duplicate class and verifying the single `order_groups` function behaves correctly. VERIFY: `pytest iptv-backend/tests/test_playlist_group_order.py -v` passes with all tests green. (cat:test; multifile:no)
