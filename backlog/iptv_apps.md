...
- [ ] [T1] iptv-backend/app/services/adult_keywords.py — Create new module exporting `MATURE_CATEGORY_KEYWORDS` as a frozenset containing the 12 terms: XXX, PORN, ADULT, EROTIC, HARDCORE, X-RATED, XRATED, 18+, HENTAI, MATURE, TV-MA, NC-17. VERIFY: `python -c "from iptv-backend.app.services.adult_keywords import MATURE_CATEGORY_KEYWORDS; assert len(MATURE_CATEGORY_KEYWORDS) == 12 and 'TV-MA' in MATURE_CATEGORY_KEYWORDS"`. (cat:python; multifile:no)
- [ ] [T2] iptv-backend/tests/test_adult_keywords.py — Add unit test asserting `MATURE_CATEGORY_KEYWORDS` from `iptv-backend/app/services/adult_keywords.py` contains all 12 expected terms and is a frozenset. VERIFY: `pytest iptv-backend/tests/test_adult_keywords.py -v`. (cat:test; multifile:no)
- [ ] [T3] iptv-backend/app/services/parental_controls.py — Remove local `MATURE_CATEGORY_KEYWORDS` definition and import it from `iptv-backend/app/services/adult_keywords.py` instead. VERIFY: `python -c "from iptv-backend.app.services.parental_controls import MATURE_CATEGORY_KEYWORDS; assert 'NC-17' in MATURE_CATEGORY_KEYWORDS"`. (cat:refactor; multifile:no)
- [ ] [T4] iptv-backend/app/routers/streams.py — Remove local `_ADULT_KEYWORDS` definition and import `MATURE_CATEGORY_KEYWORDS` from `iptv-backend/app/services/adult_keywords.py`, updating all references to use the imported constant. VERIFY: `grep -q "from iptv-backend.app.services.adult_keywords import MATURE_CATEGORY_KEYWORDS" iptv-backend/app/routers/streams.py && ! grep -q "_ADULT_KEYWORDS = {" iptv-backend/app/routers/streams.py`. (cat:refactor; multifile:no)
- [ ] [T5] iptv-backend/app/routers/search.py — Remove local `_ADULT_KEYWORDS` definition and import `MATURE_CATEGORY_KEYWORDS` from `iptv-backend/app/services/adult_keywords.py`, updating all references to use the imported constant. VERIFY: `grep -q "from iptv-backend.app.services.adult_keywords import MATURE_CATEGORY_KEYWORDS" iptv-backend/app/routers/search.py && ! grep -q "_ADULT_KEYWORDS = {" iptv-backend/app/routers/search.py`. (cat:refactor; multifile:no)
