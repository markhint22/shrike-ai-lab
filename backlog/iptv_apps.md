...
- [ ] [T3] iptv-android/app/src/main/java/com/chickadeestreams/iptv/ui/mobile/viewmodels/ProfilesViewModel.kt — **Live parental-control bypass**: `selectProfile(id)` immediately activates any profile (sets the `X-Profile-Id` header via `authInterceptor.setActiveProfileId`) with zero PIN check, even though `ProfileDto.isLocked` (from `is_locked` in `iptv-android/app/src/main/java/com/chickadeestreams/iptv/data/api/ChickadeeApi.kt:497`) is fetched and rendered. The backend implements `POST /api/profiles/{id}/unlock` (`iptv-backend/app/routers/profiles.py`, PIN-checked with lockout via `PinLockoutTracker`) and the web client enforces it correctly (`iptv-web/src/views/ProfilesView.vue`'s unlock dialog + `iptv-web/src/stores/profiles.ts::unlockProfile`), but Android's `ChickadeeApi.kt` has no `unlock` endpoint declared at all — a PIN-locked kid profile can be switched into on Android with no PIN prompt whatsoever. VERIFY: `grep -n "isLocked" iptv-android/app/src/main/java/com/chickadeestreams/iptv/ui/mobile/viewmodels/ProfilesViewModel.kt` (currently returns nothing — no lock check exists in the selection path). (cat:android; multifile:yes) [feat:iptv_apps-20260923-android-profile-pin-bypass]

- [ ] [T2] iptv-android/app/src/main/java/com/chickadeestreams/iptv/data/repository/DownloadRepository.kt — **Live functional bug**: Android downloads never leave the "requested" state. The backend's `PATCH /api/downloads/{download_id}` (`iptv-backend/app/routers/downloads.py::update_download_status`, docstring: "Client reports a download finished (ready) or failed... this just records the terminal state so it stops showing as perpetually 'requested'") exists specifically because the client is expected to call it once local packaging finishes, but `DownloadRepository.kt` only has `startDownload`/`removeDownload`/`listDownloads`/`localDownloadState`/`offlineMediaItem` — no status-update call exists in `ChickadeeApi.kt` at all (only `POST`/`GET`/`DELETE` for `/api/downloads`). Every Android-initiated download is stuck showing "requested" server-side forever. VERIFY: `grep -n "api/downloads/{downloadId}" iptv-android/app/src/main/java/com/chickadeestreams/iptv/data/api/ChickadeeApi.kt` (currently only 1 hit, the `@DELETE`; no `@PATCH`). (cat:android; multifile:yes) [feat:iptv_apps-20260923-android-download-status-never-patched]

- [ ] [T2] iptv-backend/app/services/premium.py — `apply_trial_expiry()` (called on every `GET /api/subscription/status` once a user's trial ends and they're not premium) computes "top-25 most-watched streams" by querying `UserWatchHistory` from `app/models/stream.py`, but that model/table is never written to anywhere in the codebase (confirmed: `grep -rn "UserWatchHistory(" app --include=*.py` matches only its own class definition) — real watch tracking goes through the unrelated `WatchHistory` model in `app/models/watch_history.py` instead. `rows`/`top_ids` are therefore always empty, so this function silently always falls back to its "fill remaining slots from earliest-imported streams" branch — meaning post-trial free users have their streams deactivated by import order, never by actual watch history, contradicting the function's own docstring and intent. VERIFY: `grep -rn "UserWatchHistory(" iptv-backend/app --include=*.py` (expect zero constructor call sites, proving the table is never populated). (cat:backend; multifile:no) [feat:iptv_apps-20260923-trial-expiry-dead-watchhistory-query]

- [ ] [T1] iptv-backend/app/models/stream.py — The `Program` class (line 72, `__tablename__ = "programs"`, UUID string PK) is fully orphaned dead code: it is never imported or queried anywhere outside its own definition in this file (the real, actually-used EPG-program model is the separate `EPGProgram` class in `app/models/epg.py`, used throughout `app/routers/epg.py`). The `programs` table still exists in the DB via `alembic/versions/0001_baseline_baseline_schema_matching_current_models.py`. Delete the dead class (and consider a follow-up migration to drop the unused table) or document why it's kept. VERIFY: `grep -rn "models.stream import" iptv-backend/app --include=*.py | grep -w Program` (expect zero matches — no file imports `Program` from `stream.py`). (cat:backend; multifile:no) [feat:iptv_apps-20260923-orphaned-program-model]

- [ ] [T2] iptv-android/app/src/main/java/com/chickadeestreams/iptv/data/api/ChickadeeApi.kt — `updateWatchProgress()` (`POST api/history/{streamId}/progress`, line 210) and `markWatchCompleted()` (`POST api/history/{streamId}/completed`, line 222) call backend routes that don't exist — the real backend only has `PUT /api/history/{stream_id}` (`iptv-backend/app/routers/watch_history.py::update_watch_progress`, JSON body not query params) and no `/completed` route at all (completion is derived automatically inside `update_watch_progress`/`report_heartbeat` when `progress_percent >= 90`). `HistoryRepository.kt`'s `updateProgress()`/`markAsCompleted()` wrap these two broken calls and are currently dead (only referenced from `HistoryRepositoryTest.kt`, no ViewModel/UI caller — confirmed via `grep -rn "\.updateProgress(\|\.markAsCompleted(" iptv-android/app/src/main` matching only the repository + test files) — the app works today because `PlayerActivity.kt` uses `reportHeartbeat` instead, which does match a real endpoint. Fix the two broken declarations to hit real routes, or delete this unused dead-end pair along with its test. VERIFY: `grep -n "api/history/{streamId}/progress\|api/history/{streamId}/completed" iptv-android/app/src/main/java/com/chickadeestreams/iptv/data/api/ChickadeeApi.kt` (both currently present and non-existent server-side). (cat:android; multifile:yes) [feat:iptv_apps-20260923-android-history-endpoints-mismatch]

- [ ] [T3] iptv-android/app/src/main/java/com/chickadeestreams/iptv/data/api/ChickadeeApi.kt — Android has no way to edit an existing profile: the backend's `PATCH /api/profiles/{profile_id}` (`iptv-backend/app/routers/profiles.py::update_profile`, backed by `ProfileUpdate` in `app/schemas/profile.py` with `name`/`avatar`/`is_kid`/`pin` fields — pass `pin=""` to clear a lock, 4-8 digits to set one) has no matching Retrofit declaration or UI in `ProfilesViewModel.kt`/`ProfilesScreen` — Android can only `getProfiles`/`createProfile`/`deleteProfile`. A user can never rename a profile, toggle kid-mode, or set/remove a profile PIN from the Android app after creation (must delete and recreate). VERIFY: `grep -n "profiles/{id}\"" iptv-android/app/src/main/java/com/chickadeestreams/iptv/data/api/ChickadeeApi.kt` (currently only the `@DELETE` declaration exists, no `@PATCH`). (cat:android; multifile:yes) [feat:iptv_apps-20260923-android-missing-profile-edit]

- [ ] [T3] iptv-ios/ChickadeeStreams/Views/Settings/SettingsView.swift — iOS's `ProfilesView` (mirrors web/Android per its own doc comment "Mirrors the web ProfilesView and the Android ProfilesScreen") has no profile-selection/activation mechanism at all — it only lists, adds (`create()`), and deletes (`deleteAt`) profiles; there is no `onTapGesture`, no "active profile" state, and no call anywhere in the file (or the rest of `iptv-ios/`) to `POST /api/profiles/{id}/unlock`. Unlike Android (which at least switches profiles, just without a PIN check — see the paired bypass item above), iOS cannot switch the active profile from this screen at all, so profile-scoped personalization/parental-controls have no functional entry point on iOS. VERIFY: `grep -n "onTapGesture\|unlock\|activeProfile" iptv-ios/ChickadeeStreams/Views/Settings/SettingsView.swift` (currently zero matches for selection or unlock logic in this file). (cat:ios; multifile:no) [feat:iptv_apps-20260923-ios-missing-profile-switch]
