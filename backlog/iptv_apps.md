...
- [ ] [T1] iptv-backend/app/services/content.py — Delete the dead `get_vod_catalog(db, page=1, limit=10)` stub function (always returns `[]`, zero callers anywhere in app/routers; confirmed via `grep -rn "services.content import" app/routers/*.py` returning nothing) — leave `validate_content_rights()` in the same file untouched. VERIFY: `! grep -q "def get_vod_catalog" iptv-backend/app/services/content.py`. (cat:python; multifile:no) [feat:iptv_apps-20260922-content-service-dead-vod-catalog-stub]
- [ ] [T1] iptv-backend/tests/test_content_service.py — Delete this entire test file; it exists solely to test the now-removed `get_vod_catalog` stub. VERIFY: `test ! -f iptv-backend/tests/test_content_service.py`. (cat:python; multifile:no) [feat:iptv_apps-20260922-content-service-dead-vod-catalog-stub]
- [ ] [T2] iptv-backend/app/services/content.py — Update the module docstring (currently `"""Content service for VOD catalog retrieval."""`) to `"""Content-rights validation service."""` since the file no longer retrieves any catalog. VERIFY: `grep -q "Content-rights validation" iptv-backend/app/services/content.py`. (cat:docs; multifile:no) [feat:iptv_apps-20260922-content-service-dead-vod-catalog-stub]
- [ ] [T2] iptv-backend/tests/test_playlist_rights.py — Run this file to confirm `validate_content_rights` is unaffected by the removal of the unrelated stub. VERIFY: `cd iptv-backend && pytest tests/test_playlist_rights.py -v`. (cat:test; multifile:no) [feat:iptv_apps-20260922-content-service-dead-vod-catalog-stub]
...
- [ ] [T1] iptv-backend/app/services/continue_watching_filter.py — Delete the dead `calculate_continue_watching_threshold(progress)` function (0-100-scale duplicate of the real `should_show_in_continue()`; zero callers anywhere in app/ or tests/, confirmed via `grep -rn "calculate_continue_watching_threshold" iptv-backend/app iptv-backend/tests`). VERIFY: `! grep -q "def calculate_continue_watching_threshold" iptv-backend/app/services/continue_watching_filter.py`. (cat:python; multifile:no) [feat:iptv_apps-20260922-dead-continue-watching-threshold-fn]
- [ ] [T2] iptv-backend/tests/test_continue_watching_filter.py — Run the existing suite to confirm no test referenced the removed function (none should, since it had zero callers). VERIFY: `cd iptv-backend && pytest tests/test_continue_watching_filter.py -v`. (cat:test; multifile:no) [feat:iptv_apps-20260922-dead-continue-watching-threshold-fn]
- [ ] [T2] iptv-backend/app/services/continue_watching_filter.py — Add a one-line module-docstring note that this file intentionally has exactly one continue-watching decision function (`should_show_in_continue`), to deter a future regeneration pass from reintroducing a second one. VERIFY: `grep -q "exactly one" iptv-backend/app/services/continue_watching_filter.py`. (cat:docs; multifile:no) [feat:iptv_apps-20260922-dead-continue-watching-threshold-fn]
...
- [ ] [T2] iptv-backend/app/services/stream_limit.py — Add a module-level docstring documenting that `app/services/streams.py` independently implements the same "cap concurrent sessions" concept with a different mechanism (`check_session_limit`/`enforce_session_limit`), and that whichever implementation is wired first for the P1 concurrent-stream-limit roadmap item should delete the other. VERIFY: `grep -q "app.services.streams" iptv-backend/app/services/stream_limit.py`. (cat:docs; multifile:no) [feat:iptv_apps-20260922-triple-duplicate-stream-limit-impls]
- [ ] [T2] iptv-backend/app/services/streams.py — Add the mirror-image module-level docstring pointing back at `app/services/stream_limit.py`'s `can_start`/`remaining` as the other unwired implementation of the same concept. VERIFY: `grep -q "app.services.stream_limit" iptv-backend/app/services/streams.py`. (cat:docs; multifile:no) [feat:iptv_apps-20260922-triple-duplicate-stream-limit-impls]
- [ ] [T3] iptv-backend/tests/test_stream_limit.py — Run to confirm this file's tests still pass unchanged (documentation-only change, no logic touched). VERIFY: `cd iptv-backend && pytest tests/test_stream_limit.py -v`. (cat:test; multifile:no) [feat:iptv_apps-20260922-triple-duplicate-stream-limit-impls]
- [ ] [T3] iptv-backend/tests/test_streams_session_limit.py — Run to confirm this file's 12 tests still pass unchanged. VERIFY: `cd iptv-backend && pytest tests/test_streams_session_limit.py -v`. (cat:test; multifile:no) [feat:iptv_apps-20260922-triple-duplicate-stream-limit-impls]

# --- Claude-decomposed from roadmap [2026-09-22 refuel round 3]: wire cleanDisplayTitle into 6 remaining raw-title display sites ---
- [ ] [T1] iptv-web/src/views/PlayerView.vue — Import `cleanDisplayTitle` from `@/utils/cleanDisplayTitle` and wrap the on-screen player title at `{{ streamsStore.currentStream.name }}` (line ~575) with it. VERIFY: `grep -c "cleanDisplayTitle" iptv-web/src/views/PlayerView.vue` returns >=1. (cat:vue; multifile:no) [feat:iptv_apps-20260922-clean-display-title-remaining-sites]
- [ ] [T1] iptv-web/src/views/StreamDetailView.vue — Import `cleanDisplayTitle` and wrap both the page title (`{{ streamsStore.currentStream.name }}`, line ~223) and the delete-confirmation dialog text (line ~363) with it. VERIFY: `grep -c "cleanDisplayTitle" iptv-web/src/views/StreamDetailView.vue` returns >=2. (cat:vue; multifile:no) [feat:iptv_apps-20260922-clean-display-title-remaining-sites]
- [ ] [T2] iptv-web/src/views/DiscoverView.vue — Import `cleanDisplayTitle` and wrap the channel-list item name `{{ channel.name }}` (line ~583) with it — do NOT touch the unrelated `country.name`/`cat.name` interpolations elsewhere in this file (those are filter-option labels, not stream titles). VERIFY: `grep -n "cleanDisplayTitle(channel.name)" iptv-web/src/views/DiscoverView.vue`. (cat:vue; multifile:no) [feat:iptv_apps-20260922-clean-display-title-remaining-sites]
- [ ] [T2] iptv-web/src/views/ParentalView.vue — Import `cleanDisplayTitle` and wrap `{{ stream.name }}` (line ~629) with it. VERIFY: `grep -c "cleanDisplayTitle" iptv-web/src/views/ParentalView.vue` returns >=1. (cat:vue; multifile:no) [feat:iptv_apps-20260922-clean-display-title-remaining-sites]
- [ ] [T3] iptv-web/src/components/ForYouRow.vue — Import `cleanDisplayTitle` and wrap `{{ item.name }}` (line ~66) with it. VERIFY: `grep -c "cleanDisplayTitle" iptv-web/src/components/ForYouRow.vue` returns >=1. (cat:vue; multifile:no) [feat:iptv_apps-20260922-clean-display-title-remaining-sites]
- [ ] [T3] iptv-web — Run the full vitest suite once to confirm zero regressions across all 5 edited views/components. VERIFY: `cd iptv-web && npx vitest run`. (cat:test; multifile:no) [feat:iptv_apps-20260922-clean-display-title-remaining-sites]
- [ ] [T4] iptv-web — Run `npm run type-check` (vue-tsc) to confirm the 5 new imports introduce no type errors. VERIFY: `cd iptv-web && npm run type-check`. (cat:web; multifile:no) [feat:iptv_apps-20260922-clean-display-title-remaining-sites]

# --- Claude-decomposed from roadmap [2026-09-22 refuel round 3]: cleanDisplayTitle Hello->Hey bug ---
- [ ] [T1] iptv-web/src/utils/cleanDisplayTitle.ts — Delete the `if (title.startsWith('Hello')) { title = 'Hey' + title.slice(5) }` block (lines ~10-13) — undocumented, untested, out of scope for the function's stated purpose, and corrupts any real channel name starting with "Hello". VERIFY: `! grep -q "'Hey'" iptv-web/src/utils/cleanDisplayTitle.ts`. (cat:typescript; multifile:no) [feat:iptv_apps-20260922-clean-display-title-hello-hey-bug]
- [ ] [T2] iptv-web/src/utils/__tests__/cleanDisplayTitle.test.ts — Add `it('does not alter titles starting with Hello', ...)` asserting `cleanDisplayTitle('Hello World News')` equals `'Hello World News'` unchanged. VERIFY: `cd iptv-web && npx vitest run cleanDisplayTitle -t "does not alter titles starting with Hello"`. (cat:test; multifile:no) [feat:iptv_apps-20260922-clean-display-title-hello-hey-bug]
