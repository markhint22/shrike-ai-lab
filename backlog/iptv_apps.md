...
- [ ] [T1] iptv-backend/app/services/account_export.py — Implement `get_user_data_bundle(user_id: int)` returning a dict with keys 'streams', 'watch_history', 'favorites', 'subscriptions', 'profiles' by querying the respective models filtered by user_id. VERIFY: pytest iptv-backend/tests/test_account_export.py::test_get_user_data_bundle_structure -q. (cat:python; multifile:no) [feat:iptv_apps-20260925-account-data-export-self-service-account]
- [ ] [T1] iptv-backend/app/services/account_deletion.py — Implement `execute_account_deletion(user_id: int)` that cancels active RevenueCat/Stripe subscriptions via existing services, then deletes associated rows in WatchHistory, UserFavorite, Stream, Subscription, Profile, and finally the User row. VERIFY: pytest iptv-backend/tests/test_account_deletion.py::test_execute_account_deletion_cascades -q. (cat:python; multifile:no) [feat:iptv_apps-20260925-account-data-export-self-service-account]
- [ ] [T2] iptv-backend/app/routers/account.py — Create new router with `GET /export` endpoint calling `get_user_data_bundle` and `DELETE /` endpoint calling `execute_account_deletion`, both protected by current user dependency. VERIFY: grep -q "def export_account" iptv-backend/app/routers/account.py && grep -q "def delete_account" iptv-backend/app/routers/account.py. (cat:endpoint; multifile:no) [feat:iptv_apps-20260925-account-data-export-self-service-account]
- [ ] [T3] iptv-backend/app/main.py — Import and include the `account` router from `app.routers.account` in the FastAPI app instance. VERIFY: grep -q "include_router.*account" iptv-backend/app/main.py. (cat:python; multifile:no) [feat:iptv_apps-20260925-account-data-export-self-service-account]
- [ ] [T2] iptv-web/src/api/account.ts — Create API client functions `exportAccountData()` and `deleteAccount()` hitting `/api/account/export` and `/api/account` respectively with appropriate headers. VERIFY: grep -q "exportAccountData" iptv-web/src/api/account.ts && grep -q "deleteAccount" iptv-web/src/api/account.ts. (cat:typescript; multifile:no) [feat:iptv_apps-20260925-account-data-export-self-service-account]
- [ ] [T3] iptv-web/src/views/SettingsView.vue — Add "Account Management" section with "Download my data" button triggering `exportAccountData` and "Delete my account" button opening a confirmation modal before calling `deleteAccount`. VERIFY: grep -q "Download my data" iptv-web/src/views/SettingsView.vue && grep -q "Delete my account" iptv-web/src/views/SettingsView.vue. (cat:vue; multifile:no) [feat:iptv_apps-20260925-account-data-export-self-service-account]
- [ ] [T2] iptv-backend/tests/test_account_export.py — Write unit test mocking DB session to verify `get_user_data_bundle` returns correct structure and filters by user_id. VERIFY: pytest iptv-backend/tests/test_account_export.py -q. (cat:test; multifile:no) [feat:iptv_apps-20260925-account-data-export-self-service-account]
- [ ] [T2] iptv-backend/tests/test_account_deletion.py — Write unit test mocking subscription cancellation and DB session to verify `execute_account_deletion` removes all associated records and the user. VERIFY: pytest iptv-backend/tests/test_account_deletion.py -q. (cat:test; multifile:no) [feat:iptv_apps-20260925-account-data-export-self-service-account]
...
- [ ] [T1] iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy — Create XML manifest with `NSPrivacyTracking=false`, empty `NSPrivacyCollectedDataTypes` array, and `NSPrivacyAccessedAPITypes` containing `NSPrivacyAccessedAPITypeUserDefaults` with reason `CA92.1`. VERIFY: `xmllint --noout iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy && grep -q "NSPrivacyAccessedAPITypeUserDefaults" iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy`. (cat:schema; multifile:no) [feat:iptv_apps-20260925-ios-privacy-manifest-privacyinfo-xcpriva]
- [ ] [T2] iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy — Add `NSPrivacyCollectedDataTypes` entry for `NSPrivacyCollectedDataTypeEmail` with `NSPrivacyCollectedDataTypeLinked=false` and `NSPrivacyCollectedDataTypePurposes=[NSPrivacyTracking, NSPrivacyTrackingAdvertising]`. VERIFY: `grep -q "NSPrivacyCollectedDataTypeEmail" iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy && grep -q "NSPrivacyCollectedDataTypeLinked" iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy`. (cat:schema; multifile:no) [feat:iptv_apps-20260925-ios-privacy-manifest-privacyinfo-xcpriva]
- [ ] [T3] iptv-ios/ChickadeeStreams.xcodeproj/project.pbxproj — Add `PrivacyInfo.xcprivacy` to the `ChickadeeStreams` target's `PBXBuildFile` and `PBXFileReference` sections, ensuring it is included in the "Copy Bundle Resources" build phase. VERIFY: `grep -c "PrivacyInfo.xcprivacy" iptv-ios/ChickadeeStreams.xcodeproj/project.pbxproj | grep -q "3"` (expecting at least 3 occurrences: file reference, build file, resource phase). (cat:refactor; multifile:no) [feat:iptv_apps-20260925-ios-privacy-manifest-privacyinfo-xcpriva]
- [ ] [T4] iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy — Add `NSPrivacyAccessedAPITypeFileTimestamps` to `NSPrivacyAccessedAPITypes` with reason `C617.1` (required for RevenueCat/Kingfisher file access). VERIFY: `grep -q "NSPrivacyAccessedAPITypeFileTimestamps" iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy && grep -q "C617.1" iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy`. (cat:schema; multifile:no) [feat:iptv_apps-20260925-ios-privacy-manifest-privacyinfo-xcpriva]
- [ ] [T5] iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy — Add `NSPrivacyCollectedDataTypeUsageData` with `NSPrivacyCollectedDataTypeLinked=false` and `NSPrivacyCollectedDataTypePurposes=[NSPrivacyTracking, NSPrivacyTrackingAdvertising]` for analytics/crash reporting. VERIFY: `grep -q "NSPrivacyCollectedDataTypeUsageData" iptv-ios/ChickadeeStreams/PrivacyInfo.xcprivacy`. (cat:schema; multifile:no) [feat:iptv_apps-20260925-ios-privacy-manifest-privacyinfo-xcpriva]
