...
# --- 27B-decomposed from roadmap [2026-09-21]: Build the still-unconsumed workspace Notifications Feed panel — now that the control-plane (review + tweak) [feat:gitlark-20260921-build-the-still-unconsumed-workspace-not] ---
- [ ] [T1] web/src/api/notificationsFeed.ts — Create API client module exporting `fetchNotificationsFeed(workspaceId)`, `fetchUnreadCount(workspaceId)`, and `markNotificationRead(workspaceId, eventId)` functions calling the three existing backend endpoints. VERIFY: `npx tsc --noEmit` passes and `grep -q "notifications-feed" web/src/api/notificationsFeed.ts`. (cat:typescript; multifile:no) [feat:gitlark-20260921-build-the-still-unconsumed-workspace-not]
- [ ] [T1] web/src/stores/notificationsFeed.ts — Create Pinia store with state for `events`, `unreadCount`, and actions `loadFeed`, `loadUnreadCount`, `markRead` that use the API client. VERIFY: `npx tsc --noEmit` passes and `grep -q "useNotificationsFeedStore" web/src/stores/notificationsFeed.ts`. (cat:typescript; multifile:no) [feat:gitlark-20260921-build-the-still-unconsumed-workspace-not]
- [ ] [T2] web/src/components/NotificationsFeedPanel.vue — Create Vue component that displays the list of notification events, shows unread count badge, and includes a button to mark individual events as read. VERIFY: `npx vue-tsc --noEmit` passes and `grep -q "NotificationsFeedPanel" web/src/components/NotificationsFeedPanel.vue`. (cat:vue; multifile:no) [feat:gitlark-20260921-build-the-still-unconsumed-workspace-not]
- [ ] [T3] web/src/views/WorkspaceDashboard.vue — Import and mount `NotificationsFeedPanel` in the workspace dashboard layout, passing the current `workspaceId` as a prop. VERIFY: `npx vue-tsc --noEmit` passes and `grep -q "NotificationsFeedPanel" web/src/views/WorkspaceDashboard.vue`. (cat:vue; multifile:no) [feat:gitlark-20260921-build-the-still-unconsumed-workspace-not]
- [ ] [T2] web/src/__tests__/notificationsFeedStore.spec.ts — Write unit tests for the Pinia store verifying that `loadFeed` populates state, `loadUnreadCount` updates count, and `markRead` calls the API and updates local state. VERIFY: `npx vitest run web/src/__tests__/notificationsFeedStore.spec.ts` passes. (cat:test; multifile:no) [feat:gitlark-20260921-build-the-still-unconsumed-workspace-not]
- [ ] [T2] web/src/__tests__/NotificationsFeedPanel.spec.ts — Write component tests verifying that the panel renders event list items, displays the unread count badge, and triggers `markRead` action when the read button is clicked. VERIFY: `npx vitest run web/src/__tests__/NotificationsFeedPanel.spec.ts` passes. (cat:test; multifile:no) [feat:gitlark-20260921-build-the-still-unconsumed-workspace-not]
- [ ] [T1] web/src/api/notificationsFeed.ts — Add TypeScript interfaces `NotificationEvent` and `UnreadCountResponse` matching the backend response schemas. VERIFY: `npx tsc --noEmit` passes and `grep -q "interface NotificationEvent" web/src/api/notificationsFeed.ts`. (cat:typescript; multifile:no) [feat:gitlark-20260921-build-the-still-unconsumed-workspace-not]
- [ ] [T3] web/src/components/NotificationBell.vue — Add a second tab labeled "Fleet Activity" to the existing dropdown that conditionally renders `NotificationsFeedPanel` when active. VERIFY: `npx vue-tsc --noEmit` passes and `grep -q "Fleet Activity" web/src/components/NotificationBell.vue`. (cat:vue; multifile:no) [feat:gitlark-20260921-build-the-still-unconsumed-workspace-not]
