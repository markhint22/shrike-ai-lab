...
- [ ] [T2] backend/app/services/conversation.py — In `add_message`'s `if is_ops_intent(content):` block, after `project_config` is resolved, also fetch the acting user's GitHub token via `select(User).where(User.id == user_id)` (reuse the existing `User` import) so a real `access_token` is available for the control-plane call. VERIFY: `grep -n "is_ops_intent(content)" -A 15 backend/app/services/conversation.py | grep -q "select(User)"`. (cat:python; multifile:no) [feat:gitlark-20260922-controlplane-chat-wiring]
- [ ] [T2] backend/app/services/conversation.py — Build a `ControlPlaneContext(session=db, access_token=user.github_token, workspace_id=conversation.workspace_id, user_id=str(user_id), conversation_id=conversation_id)` (import `ControlPlaneContext` from `app.services.control_plane`) right after `tool_calls = extract_tool_calls(response)`, replacing the current bare `logger.info` discard. VERIFY: `grep -q "ControlPlaneContext(" backend/app/services/conversation.py`. (cat:python; multifile:no) [feat:gitlark-20260922-controlplane-chat-wiring]
- [ ] [T2] backend/app/services/conversation.py — For each dict in `tool_calls`, `await execute_tool_call(call["name"], call["input"], ctx)` (import `execute_tool_call` from `app.services.control_plane_agent` alongside the existing `ControlPlaneAgent, extract_tool_calls` import) and collect the results into a `tool_results` list, wrapping each call in its own try/except so one failing tool doesn't drop the others' results. VERIFY: `grep -q "execute_tool_call(call\[" backend/app/services/conversation.py`. (cat:python; multifile:no) [feat:gitlark-20260922-controlplane-chat-wiring]
- [ ] [T2] backend/app/services/conversation.py — Build the response message via the already-defined `format_control_plane_response(tool_calls, tool_results)` and `conversation.messages.append(...)` it (with `flag_modified(conversation, "messages")`, matching the pattern used for the human message a few lines above) BEFORE the `await db.commit()` call, so the assistant's structured tool-result reply is actually persisted in the same turn. VERIFY: `grep -n "is_ops_intent(content)" -A 25 backend/app/services/conversation.py | grep -q "format_control_plane_response(tool_calls"`. (cat:python; multifile:no) [feat:gitlark-20260922-controlplane-chat-wiring]
- [ ] [T2] backend/tests/test_conversation_service.py — Add `test_add_message_ops_intent_appends_control_plane_response`: mock `ControlPlaneAgent.decide` to return a fake `tool_use` response for the `status` tool, mock `execute_tool_call` to return a canned status dict, call `add_message` with ops-intent content (e.g. `"what's the queue status"`), and assert the returned/refreshed conversation's last message has `role == "assistant"` and its `content` contains the tool name `"status"` (proving a real response message was appended, not just logged). VERIFY: `cd backend && python -m pytest tests/test_conversation_service.py::test_add_message_ops_intent_appends_control_plane_response -v`. (cat:python; multifile:no) [feat:gitlark-20260922-controlplane-chat-wiring]
- [ ] [T1] backend/tests/test_conversation_service.py — Add `test_add_message_ops_intent_tool_failure_still_commits`: mock `execute_tool_call` to raise, and assert `add_message` still completes without raising (the per-call try/except from step 3 swallows it) and still appends SOME response message rather than crashing the whole chat turn. VERIFY: `cd backend && python -m pytest tests/test_conversation_service.py::test_add_message_ops_intent_tool_failure_still_commits -v`. (cat:python; multifile:no) [feat:gitlark-20260922-controlplane-chat-wiring]
- [ ] [T1] backend/tests/test_conversation_ops_routing.py — Add a regression test asserting the real wiring path exists (not just the keyword gate): `import inspect; from app.services import conversation; src = inspect.getsource(conversation.ConversationService.add_message); assert "execute_tool_call" in src and "format_control_plane_response(tool_calls" in src`. VERIFY: `cd backend && python -m pytest tests/test_conversation_ops_routing.py -v`. (cat:python; multifile:no) [feat:gitlark-20260922-controlplane-chat-wiring]
...
- [ ] [T1] backend/app/services/claude_agent.py — Delete this entire file; `ClaudeAgent` duplicates the real, live, rate-limited `ClaudeClient` in `backend/app/services/claude.py` and has zero callers anywhere in `backend/app` outside its own test. VERIFY: `test ! -f backend/app/services/claude_agent.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-service-duplicates-cleanup]
- [ ] [T1] backend/tests/test_claude_agent_wrappers.py — Delete this file, the only caller of the now-deleted `ClaudeAgent`. VERIFY: `test ! -f backend/tests/test_claude_agent_wrappers.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-service-duplicates-cleanup]
- [ ] [T1] backend/app/services/github_integration_service.py — Delete this entire file; `GitHubIntegrationService` duplicates the real, live `GitHubClient` in `backend/app/services/github.py` and has zero callers anywhere in `backend/app` outside its own test. VERIFY: `test ! -f backend/app/services/github_integration_service.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-service-duplicates-cleanup]
- [ ] [T1] backend/tests/test_github_integration_service.py — Delete this file, the only caller of the now-deleted `GitHubIntegrationService`. VERIFY: `test ! -f backend/tests/test_github_integration_service.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-service-duplicates-cleanup]
- [ ] [T1] backend/app/services/plan_service.py — Delete this entire file; `PlanService.generate_plan()` is a primitive keyword-heuristic prototype superseded by the real, Claude-backed `PlannerService.generate_plan()` in `backend/app/services/planner.py`, and has zero callers anywhere in `backend/app` outside its own test. VERIFY: `test ! -f backend/app/services/plan_service.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-service-duplicates-cleanup]
- [ ] [T1] backend/tests/test_plan_service.py — Delete this file, the only caller of the now-deleted `PlanService`. VERIFY: `test ! -f backend/tests/test_plan_service.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-service-duplicates-cleanup]
- [ ] [T1] backend/tests/test_dead_service_duplicates_removed.py — Add a new regression test file asserting all three modules are actually gone from the importable namespace (not just file-deleted, catching any stray re-export): `import importlib, pytest` then `for mod in ("app.services.claude_agent", "app.services.github_integration_service", "app.services.plan_service"): with pytest.raises(ModuleNotFoundError): importlib.import_module(mod)`. VERIFY: `cd backend && python -m pytest tests/test_dead_service_duplicates_removed.py -v`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-service-duplicates-cleanup]
...
- [ ] [T1] backend/app/services/dispatch_dedup.py — Delete this entire file; `is_duplicate_dispatch`/`record_dispatch`/`filter_undispatched` have zero callers anywhere in `backend/app` outside their own test — the real dedup check lives inline in `DispatcherService.dispatch()` (`if marker in existing:`, backend/app/services/dispatcher.py). VERIFY: `test ! -f backend/app/services/dispatch_dedup.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-dispatch-scaffolding-cleanup]
- [ ] [T1] backend/tests/test_dispatch_dedup.py — Delete this file, the only caller of the now-deleted `dispatch_dedup` module. VERIFY: `test ! -f backend/tests/test_dispatch_dedup.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-dispatch-scaffolding-cleanup]
- [ ] [T1] backend/app/services/dispatcher.py — Delete the dead module-level `commit_task_to_git()` function (a SHA-256 "commit hash" generator unrelated to the real dispatch flow, which creates actual GitHub commits via `create_or_update_file`); confirmed via grep it has zero callers outside `dispatcher.py` itself and `backend/tests/test_dispatcher.py`. VERIFY: `! grep -q "^def commit_task_to_git" backend/app/services/dispatcher.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-dispatch-scaffolding-cleanup]
- [ ] [T1] backend/app/services/dispatcher.py — Delete the dead module-level `dispatch_flow()` function (only ever wraps the now-deleted `commit_task_to_git`), confirmed zero callers outside its own test. VERIFY: `! grep -q "^def dispatch_flow" backend/app/services/dispatcher.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-dispatch-scaffolding-cleanup]
- [ ] [T1] backend/app/services/dispatcher.py — Delete the dead module-level `queue_action_from_chat()` function, an early stub superseded by the real `ControlPlaneAgent`/`execute_tool_call` tool-calling flow; confirmed zero callers anywhere including its own test file (no test exercises it at all). VERIFY: `! grep -q "^def queue_action_from_chat" backend/app/services/dispatcher.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-dispatch-scaffolding-cleanup]
- [ ] [T1] backend/tests/test_dispatcher.py — Remove `test_commit_task_to_git_returns_hash` and `test_dispatch_flow_includes_commit` (and their now-broken `from app.services.dispatcher import commit_task_to_git, dispatch_flow` import) since both target functions are deleted; keep every other test in the file (real `DispatcherService`/`task_marker` coverage) untouched. VERIFY: `cd backend && python -m pytest tests/test_dispatcher.py -v`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-dispatch-scaffolding-cleanup]
- [ ] [T1] backend/app/services/dispatcher.py — Confirm `task_marker()` (the one dispatch-domain helper the real `DispatcherService.dispatch()` actually calls) is untouched and still exported, guarding against an over-eager deletion pass in the steps above. VERIFY: `grep -q "^def task_marker" backend/app/services/dispatcher.py`. (cat:python; multifile:no) [feat:gitlark-20260922-dead-dispatch-scaffolding-cleanup]
...
- [ ] [T1] backend/app/services/github_payload.py — Delete this entire file; `normalize_pull_request_payload()`'s 3-action `reviewable_actions` set (`opened`/`synchronize`/`reopened`) is a dead, narrower, DRIFTED duplicate of the live `github_webhook.py`'s correct 4-action `_REVIEWABLE_ACTIONS` (which also includes `ready_for_review`) — confirmed zero callers anywhere outside its own test. VERIFY: `test ! -f backend/app/services/github_payload.py`. (cat:python; multifile:no) [feat:gitlark-20260922-webhook-classification-dedup]
- [ ] [T1] backend/tests/test_github_payload.py — Delete this file, the only caller of the now-deleted `github_payload` module. VERIFY: `test ! -f backend/tests/test_github_payload.py`. (cat:python; multifile:no) [feat:gitlark-20260922-webhook-classification-dedup]
- [ ] [T1] backend/app/services/webhook_events.py — Delete this entire file; `classify_webhook_event()` has the same drifted 3-action reviewable set as `github_payload.py` and zero callers anywhere outside its own test — the live `github_webhook.py` handler is the single correct, already-tested source of truth for this classification. VERIFY: `test ! -f backend/app/services/webhook_events.py`. (cat:python; multifile:no) [feat:gitlark-20260922-webhook-classification-dedup]
- [ ] [T1] backend/tests/test_webhook_events.py — Delete this file, the only caller of the now-deleted `webhook_events` module. VERIFY: `test ! -f backend/tests/test_webhook_events.py`. (cat:python; multifile:no) [feat:gitlark-20260922-webhook-classification-dedup]
- [ ] [T1] backend/app/routers/github_webhook.py — Add a short comment above `_REVIEWABLE_ACTIONS` noting it is now the SOLE reviewable-actions definition in the repo (the two duplicate/drifted copies were removed), so a future change doesn't reintroduce a second inconsistent copy. VERIFY: `grep -q "sole reviewable-actions definition" backend/app/routers/github_webhook.py`. (cat:python; multifile:no) [feat:gitlark-20260922-webhook-classification-dedup]
- [ ] [T1] backend/tests/test_github_webhook.py — Confirm the existing webhook test suite (covering `_REVIEWABLE_ACTIONS` behavior for opened/synchronize/reopened/ready_for_review/closed) still passes unchanged after the deletions above, proving no real behavior depended on the removed duplicate modules. VERIFY: `cd backend && python -m pytest tests/test_github_webhook.py -v`. (cat:python; multifile:no) [feat:gitlark-20260922-webhook-classification-dedup]
...
- [ ] [T1] backend/app/services/hunk_summary.py — Delete this entire file; `summarize_diff()` re-implements the same additions/deletions counting that `diff_parser.py`'s `count_diff_changes()` already provides and whose own docstring calls itself "the single shared implementation used by the review services." Confirmed zero callers anywhere outside its own test. VERIFY: `test ! -f backend/app/services/hunk_summary.py`. (cat:python; multifile:no) [feat:gitlark-20260922-hunk-summary-dedup]
- [ ] [T1] backend/tests/test_hunk_summary.py — Delete this file, the only caller of the now-deleted `hunk_summary` module. VERIFY: `test ! -f backend/tests/test_hunk_summary.py`. (cat:python; multifile:no) [feat:gitlark-20260922-hunk-summary-dedup]
