...
- [ ] [T1] backend/app/services/feature_flags.py — Verify `FeatureFlagService` class exists with methods `create_flag`, `is_enabled`, `update_rollout`, and `get_all_flags`. VERIFY: `grep -q "class FeatureFlagService" backend/app/services/feature_flags.py && grep -q "def create_flag" backend/app/services/feature_flags.py && grep -q "def is_enabled" backend/app/services/feature_flags.py && grep -q "def update_rollout" backend/app/services/feature_flags.py && grep -q "def get_all_flags" backend/app/services/feature_flags.py`. (cat:python; multifile:no)
- [ ] [T2] backend/app/services/feature_flags_service.py — Delete the file containing the dead `FeatureFlagsService` implementation. VERIFY: `test ! -f backend/app/services/feature_flags_service.py`. (cat:refactor; multifile:no)
- [ ] [T3] backend/app/routers/feature_flags.py — Create new router file defining `APIRouter` and a `GET /feature-flags` endpoint that instantiates `FeatureFlagService` and returns the result of `get_all_flags()`. VERIFY: `grep -q "from fastapi import APIRouter" backend/app/routers/feature_flags.py && grep -q "@router.get(\"/feature-flags\")" backend/app/routers/feature_flags.py && grep -q "FeatureFlagService" backend/app/routers/feature_flags.py`. (cat:endpoint; multifile:no)
- [ ] [T4] backend/app/main.py — Import the new `feature_flags` router and include it in the FastAPI app with prefix `/api`. VERIFY: `grep -q "from app.routers import feature_flags" backend/app/main.py && grep -q "app.include_router(feature_flags.router, prefix=\"/api\")" backend/app/main.py`. (cat:endpoint; multifile:yes)
- [ ] [T5] backend/tests/test_feature_flags.py — Create a test file that mocks the database session and asserts `FeatureFlagService.get_all_flags()` returns a list and `is_enabled` respects rollout percentage logic. VERIFY: `python -m pytest backend/tests/test_feature_flags.py -v`. (cat:test; multifile:no)
...
- [ ] [T1] web/src/components/ReviewCollaborationManager.vue — Update `wsUrl` construction to include the `/ws` segment: `ws://${window.location.host}/api/ws/reviews/ws/${props.reviewId}`. VERIFY: `grep -q "api/ws/reviews/ws/" web/src/components/ReviewCollaborationManager.vue`. (cat:vue; multifile:no)
- [ ] [T2] web/src/components/__tests__/ReviewCollaborationManager.spec.ts — Create a unit test that mocks `window.location` and asserts the constructed WebSocket URL contains `/api/ws/reviews/ws/{id}`. VERIFY: `npx vitest run web/src/components/__tests__/ReviewCollaborationManager.spec.ts`. (cat:test; multifile:no)
- [ ] [T3] web/src/components/EnhancedCodeReview.vue — Import `ReviewCollaborationManager` and add `<ReviewCollaborationManager :review-id="props.reviewId" :user-id="userId" :user-name="userName" />` to the template, ensuring `userId` and `userName` are available in scope (from auth store or props). VERIFY: `grep -q "ReviewCollaborationManager" web/src/components/EnhancedCodeReview.vue && grep -q "<ReviewCollaborationManager" web/src/components/EnhancedCodeReview.vue`. (cat:vue; multifile:no)
- [ ] [T4] web/src/components/__tests__/EnhancedCodeReview.spec.ts — Create a unit test that mounts `EnhancedCodeReview` with mock props and asserts that `ReviewCollaborationManager` is rendered in the DOM. VERIFY: `npx vitest run web/src/components/__tests__/EnhancedCodeReview.spec.ts`. (cat:test; multifile:no)
- [ ] [T5] backend/app/routers/review_collab.py — Verify the WebSocket route path is exactly `/ws/{review_id}` under the prefix `/api/ws/reviews` to confirm the full path is `/api/ws/reviews/ws/{review_id}`. VERIFY: `python -c "from backend.app.routers.review_collab import router; print([r.path for r in router.routes if hasattr(r, 'path')])" | grep "/ws/{review_id}"`. (cat:endpoint; multifile:no)
...
- [ ] [T1] vscode-extension/src/services/api-client.ts — Update `starSnippet` to accept a `starred: boolean` parameter and include `{ starred }` in the POST body. VERIFY: `grep -q "body: JSON.stringify({ starred" vscode-extension/src/services/api-client.ts`. (cat:typescript; multifile:no)
- [ ] [T2] vscode-extension/src/test/unit/api-client.test.ts — Add unit test mocking fetch to verify `starSnippet` sends correct body and `deleteSnippet` uses DELETE method. VERIFY: `npx jest vscode-extension/src/test/unit/api-client.test.ts --passWithNoTests`. (cat:test; multifile:no)
- [ ] [T3] vscode-extension/src/extension-core.ts — Register `gitlark.starSnippet` and `gitlark.deleteSnippet` commands that call the updated API client methods. VERIFY: `grep -q "registerCommand('gitlark.starSnippet'" vscode-extension/src/extension-core.ts`. (cat:typescript; multifile:no)
- [ ] [T4] vscode-extension/src/views/snippets-tree.ts — Add `starred` property to `SnippetTreeItem` and update `contextValue` to distinguish starred/unstarred states. VERIFY: `grep -q "contextValue.*starred" vscode-extension/src/views/snippets-tree.ts`. (cat:typescript; multifile:no)
- [ ] [T5] vscode-extension/package.json — Add `gitlark.starSnippet` and `gitlark.deleteSnippet` to `contributes.commands` and `view/item/context` menu for `gitlarkSnippets` tree items. VERIFY: `jq '.contributes["view/item/context"] | length > 0' vscode-extension/package.json`. (cat:typescript; multifile:no)
