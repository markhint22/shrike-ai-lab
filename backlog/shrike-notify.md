...
- [ ] [T1] backend/app/services/auth.py — Add `bootstrap_token_exists()` helper that checks if any wildcard token exists in the store. VERIFY: python -m pytest backend/tests/test_auth.py::test_bootstrap_token_exists -v. (cat:python; multifile:no) [feat:shrike-notify-20260921-close-the-token-issuance-auth-bypass-pos]
- [ ] [T2] backend/app/routers/messages.py — Modify `issue_token` endpoint to accept `require_wildcard_token` dependency conditionally based on `settings.require_auth` and `bootstrap_token_exists()`. VERIFY: python -m pytest backend/tests/test_tokens_api.py::test_post_tokens_requires_auth_when_bootstrapped -v. (cat:endpoint; multifile:no) [feat:shrike-notify-20260921-close-the-token-issuance-auth-bypass-pos]
- [ ] [T3] backend/scripts/token_mint.py — Update CLI to call `issue_token` directly for initial bootstrap, ensuring it works without HTTP auth context. VERIFY: python -m pytest backend/tests/test_token_mint.py::test_cli_bootstrap_mint -v. (cat:python; multifile:no) [feat:shrike-notify-20260921-close-the-token-issuance-auth-bypass-pos]
- [ ] [T4] backend/tests/test_tokens_api.py — Add regression test asserting `POST /tokens` returns 401 when `NOTIFY_REQUIRE_AUTH=1` and a wildcard token already exists. VERIFY: python -m pytest backend/tests/test_tokens_api.py::test_post_tokens_401_unauth_when_bootstrapped -v. (cat:test; multifile:no) [feat:shrike-notify-20260921-close-the-token-issuance-auth-bypass-pos]
- [ ] [T5] backend/tests/test_tokens_api.py — Add test asserting `POST /tokens` returns 201 when `NOTIFY_REQUIRE_AUTH=1` but no wildcard token exists yet (bootstrap case). VERIFY: python -m pytest backend/tests/test_tokens_api.py::test_post_tokens_201_bootstrap_when_no_token -v. (cat:test; multifile:no) [feat:shrike-notify-20260921-close-the-token-issuance-auth-bypass-pos]
