...
- [ ] [T1] backend/tests/test_cross_repo_publish_contract.py — Create a new test file containing a `test_overnight_queue_payload_shape` function that defines the exact hardcoded JSON payload `{"title": "Test", "body": "Body", "tags": ["tag1"], "priority": "default"}` and asserts it passes validation against the `PublishRequest` schema without raising exceptions. VERIFY: `cd backend && python -m pytest tests/test_cross_repo_publish_contract.py::test_overnight_queue_payload_shape -v`. (cat:test; multifile:no) [feat:shrike-notify-20260920-cross-repo-publish-contract-regression-t]
- [ ] [T2] backend/tests/test_cross_repo_publish_contract.py — Add a `test_overnight_queue_endpoint_acceptance` test that uses the FastAPI TestClient to POST the hardcoded payload to a valid topic endpoint, mocking the broker dependency, and asserts the response status code is 200 or 202. VERIFY: `cd backend && python -m pytest tests/test_cross_repo_publish_contract.py::test_overnight_queue_endpoint_acceptance -v`. (cat:test; multifile:no) [feat:shrike-notify-20260920-cross-repo-publish-contract-regression-t]
- [ ] [T3] backend/tests/test_cross_repo_publish_contract.py — Add a docstring to the test module explicitly documenting the dependency on `shared/scripts/overnight-queue/shrike_notify_lib.sh` and warning against modifying `PublishRequest` required fields or keys without updating this contract test. VERIFY: `grep -q "shrike_notify_lib.sh" backend/tests/test_cross_repo_publish_contract.py`. (cat:docs; multifile:no) [feat:shrike-notify-20260920-cross-repo-publish-contract-regression-t]
- [ ] [T4] backend/tests/test_cross_repo_publish_contract.py — Add a negative test `test_overnight_queue_rejects_missing_priority` that verifies the endpoint returns 422 if the `priority` field is omitted, ensuring the contract's strictness on required fields is pinned. VERIFY: `cd backend && python -m pytest tests/test_cross_repo_publish_contract.py::test_overnight_queue_rejects_missing_priority -v`. (cat:test; multifile:no) [feat:shrike-notify-20260920-cross-repo-publish-contract-regression-t]
- [ ] [T5] backend/tests/test_cross_repo_publish_contract.py — Add a negative test `test_overnight_queue_rejects_invalid_tags_type` that verifies the endpoint returns 422 if `tags` is provided as a string instead of a list, pinning the type constraint of the cross-repo contract. VERIFY: `cd backend && python -m pytest tests/test_cross_repo_publish_contract.py::test_overnight_queue_rejects_invalid_tags_type -v`. (cat:test; multifile:no) [feat:shrike-notify-20260920-cross-repo-publish-contract-regression-t]
...
- [ ] [T1] backend/scripts/token_mint.py — Add `generate_token_payload(scope: str) -> dict` helper that returns a dict with `scope` and a placeholder `token` field for validation logic. VERIFY: python -m pytest backend/tests/test_token_mint.py::test_generate_token_payload_scope -v. (cat:python; multifile:no) [feat:shrike-notify-20260920-token-minting-rotation-helper-for-automa]
- [ ] [T1] backend/scripts/token_mint.py — Add `validate_scope(scope: str) -> bool` function that accepts "*" or specific topic slugs and rejects invalid characters. VERIFY: python -m pytest backend/tests/test_token_mint.py::test_validate_scope_wildcard_and_specific -v. (cat:python; multifile:no) [feat:shrike-notify-20260920-token-minting-rotation-helper-for-automa]
- [ ] [T2] backend/scripts/token_mint.py — Implement `mint_tokens(scopes: list[str]) -> list[dict]` that generates a list of token payloads for the provided scopes, raising ValueError if any scope fails validation. VERIFY: python -m pytest backend/tests/test_token_mint.py::test_mint_tokens_multiple_scopes -v. (cat:python; multifile:no) [feat:shrike-notify-20260920-token-minting-rotation-helper-for-automa]
- [ ] [T2] backend/scripts/token_mint.py — Add `rotate_token(old_token: str, new_scope: str) -> dict` helper that returns a new token payload and marks the old one for revocation in the response metadata. VERIFY: python -m pytest backend/tests/test_token_mint.py::test_rotate_token_structure -v. (cat:python; multifile:no) [feat:shrike-notify-20260920-token-minting-rotation-helper-for-automa]
- [ ] [T3] backend/scripts/token_mint.py — Create CLI entry point using `argparse` to accept `--scope` (repeatable) and `--rotate` flags, printing JSON output of minted tokens. VERIFY: python backend/scripts/token_mint.py --scope "*" --scope "fleet_queue_task" | jq '.[0].scope' == "*". (cat:python; multifile:no) [feat:shrike-notify-20260920-token-minting-rotation-helper-for-automa]
- [ ] [T1] backend/tests/test_token_mint.py — Create test file with unit tests for `generate_token_payload`, `validate_scope`, `mint_tokens`, and `rotate_token` covering wildcard, specific topics, and invalid inputs. VERIFY: python -m pytest backend/tests/test_token_mint.py -v. (cat:test; multifile:no) [feat:shrike-notify-20260920-token-minting-rotation-helper-for-automa]
- [ ] [T2] backend/scripts/README.md — Add "Token Management" section documenting the security tradeoff between wildcard (`*`) and per-topic scoping, including example commands for both strategies. VERIFY: grep -q "wildcard" backend/scripts/README.md && grep -q "per-topic" backend/scripts/README.md. (cat:docs; multifile:no) [feat:shrike-notify-20260920-token-minting-rotation-helper-for-automa]
- [ ] [T3] backend/scripts/pre_commit_check.py — Update pre-commit check to ensure `backend/scripts/token_mint.py` is executable and passes syntax check via `py_compile`. VERIFY: python backend/scripts/pre_commit_check.py && echo "PASS". (cat:python; multifile:no) [feat:shrike-notify-20260920-token-minting-rotation-helper-for-automa]
