...
- [ ] [T1] backend/tests/test_no_duplicate_ttl_module.py — Create new test file that asserts `importlib.util.find_spec("app.utils.ttl")` returns None and `os.path.exists("backend/app/utils/ttl.py")` is False. VERIFY: `pytest backend/tests/test_no_duplicate_ttl_module.py -v`. (cat:test; multifile:no) [feat:shrike-notify-20260925-finally-close-the-oscillating-backend-ap]
- [ ] [T2] backend/app/utils/ttl.py — Delete the dead duplicate module file entirely from the filesystem. VERIFY: `test ! -f backend/app/utils/ttl.py && echo "PASS"`. (cat:refactor; multifile:no) [feat:shrike-notify-20260925-finally-close-the-oscillating-backend-ap]
- [ ] [T3] backend/tests/test_no_duplicate_ttl_module.py — Add a second assertion in the new test that verifies `importlib.util.find_spec("app.services.ttl")` is NOT None to ensure the real module remains intact. VERIFY: `pytest backend/tests/test_no_duplicate_ttl_module.py -v`. (cat:test; multifile:no) [feat:shrike-notify-20260925-finally-close-the-oscillating-backend-ap]
- [ ] [T4] backend/scripts/pre_commit_check.py — Add a check that fails the pre-commit hook if `backend/app/utils/ttl.py` exists, preventing silent recreation. VERIFY: `touch backend/app/utils/ttl.py && python backend/scripts/pre_commit_check.py; test $? -ne 0`. (cat:refactor; multifile:no) [feat:shrike-notify-20260925-finally-close-the-oscillating-backend-ap]
- [ ] [T5] backend/tests/test_no_duplicate_ttl_module.py — Add a docstring explaining the historical oscillation issue and referencing the 2026-09-21 roadmap item for context. VERIFY: `grep -q "oscillating" backend/tests/test_no_duplicate_ttl_module.py`. (cat:docs; multifile:no) [feat:shrike-notify-20260925-finally-close-the-oscillating-backend-ap]
