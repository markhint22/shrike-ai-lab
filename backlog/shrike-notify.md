...
- [ ] [T2] backend/tests/test_notify_cli_watch.py — Add tests for the `watch` subcommand argument parsing and basic execution flow with mocked SSE client. VERIFY: python -m pytest backend/tests/test_notify_cli_watch.py -v. (cat:test; multifile:no) [feat:shrike-notify-20260925-add-a-tail-watch-subcommand-to-backend-s]
...
- [ ] [T1] backend/app/utils/coalesce.py — Verify `first_present()` signature accepts variable args and returns first non-None value or default. VERIFY: python -m pytest backend/tests/test_coalesce.py::test_first_present_basic -v. (cat:python; multifile:no) [feat:shrike-notify-20260925-re-verify-and-either-wire-or-drop-backen]
- [ ] [T2] backend/tests/test_config.py — Add test asserting `message_ttl_seconds` uses `first_present` for env var resolution logic. VERIFY: python -m pytest backend/tests/test_config.py::test_message_ttl_uses_coalesce -v. (cat:test; multifile:no) [feat:shrike-notify-20260925-re-verify-and-either-wire-or-drop-backen]
- [ ] [T3] backend/app/config.py — Refactor `message_ttl_seconds` property to call `utils.coalesce.first_present(os.getenv("MESSAGE_TTL_SECONDS"), os.getenv("NOTIFY_TTL_SECONDS"), default=3600)`. VERIFY: python -m pytest backend/tests/test_config.py -v. (cat:refactor; multifile:no) [feat:shrike-notify-20260925-re-verify-and-either-wire-or-drop-backen]
- [ ] [T4] backend/app/config.py — Remove manual nested `os.getenv` logic for TTL fallback, ensuring no duplicate import of `coalesce`. VERIFY: grep -q "first_present" backend/app/config.py && ! grep -q "if os.getenv" backend/app/config.py. (cat:refactor; multifile:no) [feat:shrike-notify-20260925-re-verify-and-either-wire-or-drop-backen]
- [ ] [T5] backend/tests/test_config.py — Add test verifying fallback to default when both env vars are unset. VERIFY: python -m pytest backend/tests/test_config.py::test_message_ttl_default_fallback -v. (cat:test; multifile:no) [feat:shrike-notify-20260925-re-verify-and-either-wire-or-drop-backen]
