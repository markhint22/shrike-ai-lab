...
- [ ] [T1] frontend/src/utils/apiClient.js — Add `getDashboardSummary` method to the API client that issues a GET request to `/api/test-runs/dashboard/summary` and returns the parsed JSON response. VERIFY: `npx vitest run src/utils/apiClient.test.js -t "getDashboardSummary"`. (cat:typescript; multifile:no)
- [ ] [T1] frontend/src/stores/testRuns.js — Add a `fetchDashboardSummary` action that calls `api.getDashboardSummary()` and stores the result in a new `dashboardSummary` state property, handling loading and error states. VERIFY: `npx vitest run src/stores/testRuns.test.js -t "fetchDashboardSummary"`. (cat:typescript; multifile:no)
