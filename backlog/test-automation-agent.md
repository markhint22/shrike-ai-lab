...
- [ ] [T1] backend/app/utils/ordinal.py, backend/tests/test_utils_ordinal.py, backend/app/utils/split_name.py, backend/tests/test_utils_env_flag.py — all four are git-tracked as 0-1 byte empty files (`git ls-files` lists them; `wc -c` returns 0 or 1), leftovers from two separate "remove dead utility" commits (c9e6602 "chore: remove unused ordinal utility and its tests", 29a6ac9 "chore: remove dead utility modules and their orphaned tests") that emptied file contents instead of `git rm`-ing them — the same "dead-code removal empties instead of git rm" bug class already found and fixed once on pagination.py/ssrf.py (see OVERNIGHT_PROGRESS.md line 1057). `git rm` all four tracked stubs. VERIFY: `! git ls-files backend/app/utils/ordinal.py backend/app/utils/split_name.py backend/tests/test_utils_ordinal.py backend/tests/test_utils_env_flag.py | grep .`. (cat:infra; multifile:yes) [feat:test-automation-agent-20260924-remove-tracked-empty-stub-files]

- [ ] [T1] backend/tests/__pycache__/test_split_name.cpython-312-pytest-7.4.4.pyc — stale compiled bytecode left behind after test_split_name.py's source was deleted from git (the .py is already gone, only the compiled artifact remains on disk), the same artifact class already being cleaned up for test_ssrf.py elsewhere in the active queue. VERIFY: `! find backend/tests/__pycache__ -name "test_split_name.*" | grep -q .`. (cat:infra; multifile:no) [feat:test-automation-agent-20260924-remove-stale-split-name-pyc]

- [ ] [T1] backend/app/utils/chunking.py — `chunk_list()` (line 7) has zero callers anywhere in backend/app and no dedicated test file, and duplicates `chunk()` in backend/app/utils/list_chunk.py (which does have backend/tests/test_utils_list_chunk.py). A prior decomposition round (2026-09-21/22, "Remove duplicate chunk()/chunk_list() (list_chunk.py vs chunking.py)") targeted this exact pair but never actually landed — both files still exist unchanged. Delete chunking.py, keeping list_chunk.py as the tested survivor. VERIFY: `! git ls-files backend/app/utils/chunking.py | grep .` && `cd backend && python -m pytest tests/test_utils_list_chunk.py -q`. (cat:backend; multifile:no) [feat:test-automation-agent-20260924-remove-dead-chunking-dup]

- [ ] [T1] backend/app/utils/ratio.py — `pass_ratio()` and `to_percent()` (lines 4, 19) have zero callers anywhere in backend/app; the pass-rate math actually wired into routers/test_runs.py and services/flake_detection.py lives in backend/app/utils/flake_ratio.py (`calculate_flake_ratio`), a separate file. Only backend/tests/test_utils_ratio.py exercises ratio.py, testing it in isolation. Delete both. VERIFY: `! git ls-files backend/app/utils/ratio.py backend/tests/test_utils_ratio.py | grep .`. (cat:backend; multifile:yes) [feat:test-automation-agent-20260924-remove-dead-ratio-util]

- [ ] [T1] backend/app/utils/count_words.py — `count_words()` (line 4) has zero callers anywhere in the app and no dedicated test file at all — fully unused and fully untested. Delete. VERIFY: `! git ls-files backend/app/utils/count_words.py | grep .`. (cat:backend; multifile:no) [feat:test-automation-agent-20260924-remove-dead-count-words]

- [ ] [T1] backend/app/utils/hostname.py — `extract_host()` (line 7) has zero callers; the one place a URL hostname actually gets extracted for real work, backend/app/utils/ssrf_guard.py (lines 21-22), does its own inline `urlparse(url).hostname` instead of calling this helper. Delete the unused module and its test, or wire ssrf_guard.py to use it if the duplication is intentional. VERIFY: `! git ls-files backend/app/utils/hostname.py backend/tests/test_utils_hostname.py | grep .`. (cat:backend; multifile:yes) [feat:test-automation-agent-20260924-remove-or-wire-dead-hostname-util]

- [ ] [T2] backend/app/utils/clamp.py `clamp_status_color()` (lines 29-46) vs backend/app/utils/status_color.py `color_for_status()`/`get_status_color` (lines 14-26) — two independently-maintained status-to-hex-color maps that have DRIFTED: clamp.py maps `"skipped"` to yellow `#eab308` and has no `"cancelled"` entry, while status_color.py maps `"pending"` to yellow `#eab308` and adds a `"cancelled"` entry (`#6b7280`) with no `"skipped"` entry at all — the same color means different things depending on which file a future caller picks. Neither function has a production caller outside its own tests (test_utils_clamp.py, test_clamp_status_color.py, test_status_color.py, test_utils_status_color.py). Consolidate into one map covering the union of both status sets, delete the redundant file, and update tests. VERIFY: `grep -l "def clamp_status_color\|def color_for_status" backend/app/utils/*.py | wc -l | grep -q '^1$'`. (cat:backend; multifile:yes) [feat:test-automation-agent-20260924-consolidate-drifted-status-color-maps]

- [ ] [T1] backend/app/utils/test_runs_query.py — `buildTestRunsQueryParams()` (the only camelCase-named function in this otherwise snake_case Python codebase) has zero callers in backend/app/routers or backend/app/services; the real list-runs endpoint (backend/app/routers/test_runs.py) builds its query filters inline. A currently-queued item (OVERNIGHT_PROGRESS.md line 974, feat:test-automation-agent-20260921-two-aider-authored-files-landed-inside-b) already assumes this function is "deleted," but the module itself was never removed — only its call site was cleaned up. Delete the orphaned module and backend/tests/test_test_runs_query.py. VERIFY: `! git ls-files backend/app/utils/test_runs_query.py backend/tests/test_test_runs_query.py | grep .`. (cat:backend; multifile:yes) [feat:test-automation-agent-20260924-remove-dead-test-runs-query-util]

- [ ] [T2] backend/app/utils/upload_status.py — `extract_upload_metadata()` and `is_valid_upload_status()`/`VALID_UPLOAD_STATUSES` (lines 7, 54-56) form a second, unwired upload-status validation path. The real upload flow validates status via the `UploadStatus` SQLAlchemy enum directly (backend/app/routers/uploads.py `list_uploads`, ~line 197: `UploadStatus(status)` in a try/except), never calling `is_valid_upload_status` or referencing `VALID_UPLOAD_STATUSES`. Both are exercised only by backend/tests/test_upload_status_utils.py and (partially) test_upload_status.py, never by production code. Either wire `is_valid_upload_status` into the real status-filter validation in `list_uploads` (replacing the raw enum try/except with a clearer pre-check), or delete the dead module and its dedicated test file. VERIFY: `grep -n "is_valid_upload_status" backend/app/routers/uploads.py backend/app/services/uploads.py` (currently empty — confirms no production caller); after the fix either a real caller appears or the module + its dedicated test are gone. (cat:backend; multifile:yes) [feat:test-automation-agent-20260924-wire-or-remove-dead-upload-status-validator]
