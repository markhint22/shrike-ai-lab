...
- [ ] [T1] backend/app/utils/pct.py — Confirm zero production callers before deleting: run a repo-wide grep for any import of this module outside its own file/tests. VERIFY: `! grep -rn "from app.utils.pct import\|app\.utils\.pct\b" backend/app --include='*.py' | grep -v 'backend/app/utils/pct.py'`. (cat:backend; multifile:no) [feat:test-automation-agent-20260922-fix-empty-test-pct-and-delete-dead-pct]
- [ ] [T1] backend/app/utils/pct.py — Delete this dead, zero-caller file (git rm). VERIFY: `test ! -f backend/app/utils/pct.py`. (cat:backend; multifile:no) [feat:test-automation-agent-20260922-fix-empty-test-pct-and-delete-dead-pct]
- [ ] [T1] backend/tests/test_pct.py — Delete this 0-byte stub test file (it currently gives false-green confidence that pct.py is tested). VERIFY: `test ! -f backend/tests/test_pct.py`. (cat:test; multifile:no) [feat:test-automation-agent-20260922-fix-empty-test-pct-and-delete-dead-pct]
- [ ] [T1] backend/app/utils/test_pct.py — If a leftover misplaced copy still exists from the earlier relocation attempt, delete it too so no third copy lingers. VERIFY: `test ! -f backend/app/utils/test_pct.py`. (cat:backend; multifile:no) [feat:test-automation-agent-20260922-fix-empty-test-pct-and-delete-dead-pct]
- [ ] [T2] backend/tests — Run the full backend test suite once to confirm removing pct.py and its stub test caused zero regressions. VERIFY: `cd backend && python -m pytest -q`. (cat:test; multifile:no) [feat:test-automation-agent-20260922-fix-empty-test-pct-and-delete-dead-pct]
...
- [ ] [T2] frontend/src/stores/flakes.js — Add a `suiteHealth` ref and a `fetchSuiteHealth()` action that GETs `/api/flakes/analysis` and stores its `suite_health` field (do not remove the existing `overallHealth` computed yet). VERIFY: `grep -q "suiteHealth" frontend/src/stores/flakes.js`. (cat:web; multifile:no) [feat:test-automation-agent-20260922-wire-real-suite-health-into-dashboard]
- [ ] [T2] frontend/src/stores/__tests__/flakes.test.js — Add a test that `fetchSuiteHealth()` populates `suiteHealth` from the mocked `/api/flakes/analysis` response's `suite_health` field. VERIFY: `cd frontend && npx vitest run src/stores/__tests__/flakes.test.js`. (cat:test; multifile:no) [feat:test-automation-agent-20260922-wire-real-suite-health-into-dashboard]
- [ ] [T3] frontend/src/stores/flakes.js — Remove the homemade `overallHealth` computed (lines computing health from avg flake_score) now that `suiteHealth` sources the real backend value; update any internal references to use `suiteHealth` instead. VERIFY: `! grep -q "overallHealth" frontend/src/stores/flakes.js`. (cat:web; multifile:no) [feat:test-automation-agent-20260922-wire-real-suite-health-into-dashboard]
- [ ] [T2] frontend/src/pages/FlakeDashboardPage.vue — Import `suiteBadge` from `../utils/suiteBadge` and call `store.fetchSuiteHealth()` alongside the existing `fetchFlakes()` call on mount. VERIFY: `grep -q "suiteBadge" frontend/src/pages/FlakeDashboardPage.vue`. (cat:web; multifile:no) [feat:test-automation-agent-20260922-wire-real-suite-health-into-dashboard]
- [ ] [T3] frontend/src/pages/FlakeDashboardPage.vue — Replace the hand-written `.health-badge.healthy/.warning/.critical` class selection with `suiteBadge(store.suiteHealth).label`/`.color` (healthy/flaky/degraded vocabulary), updating the template markup accordingly. VERIFY: `grep -q "suiteBadge(" frontend/src/pages/FlakeDashboardPage.vue`. (cat:web; multifile:no) [feat:test-automation-agent-20260922-wire-real-suite-health-into-dashboard]
- [ ] [T2] frontend/src/pages/__tests__/FlakeDashboardPage.spec.js — Add/update a test asserting the health badge renders the label produced by `suiteBadge()` for a mocked `suite_health` value of `"degraded"`. VERIFY: `cd frontend && npx vitest run src/pages/__tests__/FlakeDashboardPage.spec.js`. (cat:test; multifile:no) [feat:test-automation-agent-20260922-wire-real-suite-health-into-dashboard]
...
- [ ] [T1] frontend/src/utils/__tests__/pct.test.ts — Create this new test file with a first passing assertion importing `pct` from `../pct` and checking `pct(1, 2) === 50`. VERIFY: `cd frontend && npx vitest run src/utils/__tests__/pct.test.ts`. (cat:test; multifile:no) [feat:test-automation-agent-20260922-resolve-duplicate-pct-js-ts]
- [ ] [T2] frontend/src/utils/__tests__/pct.test.ts — Add a case for `pct(0, 0) === 0` (zero-denominator guard) and a rounding-boundary case `pct(1, 3) === 33`. VERIFY: `cd frontend && npx vitest run src/utils/__tests__/pct.test.ts`. (cat:test; multifile:no) [feat:test-automation-agent-20260922-resolve-duplicate-pct-js-ts]
- [ ] [T1] frontend/src/utils/pct.js — Delete this duplicate plain-JS file now that `pct.ts` is the sole, tested implementation. VERIFY: `test ! -f frontend/src/utils/pct.js`. (cat:web; multifile:no) [feat:test-automation-agent-20260922-resolve-duplicate-pct-js-ts]
- [ ] [T2] frontend/src/stores/testRuns.js — Confirm the existing `import { pct } from '../utils/pct'` still resolves cleanly to `pct.ts` after deleting `pct.js` (no import path change needed since both were extensionless). VERIFY: `cd frontend && npx vitest run src/stores/__tests__/testRuns.test.js`. (cat:web; multifile:no) [feat:test-automation-agent-20260922-resolve-duplicate-pct-js-ts]
