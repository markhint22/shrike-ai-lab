...
# --- 27B-decomposed from roadmap [2026-09-21]: The GDPR/CCPA "export my data" and "delete my account" endpoints are 100% fake — `app/serv (review + tweak) [feat:billwatch-20260921-the-gdpr-ccpa-export-my-data-and-delete-] ---
- [ ] [T1] billwatch-backend/app/models/export_request.py — Create `ExportRequest` ORM model with `id`, `user_id`, `status` (enum: processing/completed/failed), `created_at`, and `completed_at` columns. VERIFY: `cd billwatch-backend && python -c "from app.models.export_request import ExportRequest; print(ExportRequest.__tablename__)"`. (cat:schema; multifile:no) [feat:billwatch-20260921-the-gdpr-ccpa-export-my-data-and-delete-]
- [ ] [T2] billwatch-backend/alembic/versions/0004_add_export_request_table.py — Create Alembic migration to add `export_requests` table matching the new model. VERIFY: `cd billwatch-backend && alembic upgrade head && alembic downgrade -1 && alembic upgrade head`. (cat:schema; multifile:no) [feat:billwatch-20260921-the-gdpr-ccpa-export-my-data-and-delete-]
- [ ] [T3] billwatch-backend/app/services/data_export_service.py — Refactor `DataExportService` to accept a `db: AsyncSession` in `__init__`, implement `request_export` to persist an `ExportRequest` row and return its ID, and `get_export_status` to query the DB for real status. VERIFY: `cd billwatch-backend && python -m pytest tests/test_data_export_service.py::test_request_export_persists_row -v`. (cat:python; multifile:no) [feat:billwatch-20260921-the-gdpr-ccpa-export-my-data-and-delete-]
- [ ] [T4] billwatch-backend/app/services/data_export_service.py — Implement `delete_account` to delete the `User` row and cascade-delete owned `BillFollow`, `Alert`, `SavedSearch`, and `Notification` rows within a single transaction. VERIFY: `cd billwatch-backend && python -m pytest tests/test_data_export_service.py::test_delete_account_cascades_owned_rows -v`. (cat:python; multifile:no) [feat:billwatch-20260921-the-gdpr-ccpa-export-my-data-and-delete-]
- [ ] [T5] billwatch-backend/tests/test_data_export_service.py — Rewrite tests to use a seeded in-memory SQLite fixture, asserting that `request_export` creates a DB row and `delete_account` removes user data from the DB. VERIFY: `cd billwatch-backend && python -m pytest tests/test_data_export_service.py -v`. (cat:test; multifile:no) [feat:billwatch-20260921-the-gdpr-ccpa-export-my-data-and-delete-]
