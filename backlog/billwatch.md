...
- [ ] [T4] billwatch-web/src/pages/SearchPage.vue — Refactor the search request logic to use a dedicated service function that encapsulates the correct API path. VERIFY: grep -q "searchService" billwatch-web/src/pages/SearchPage.vue && grep -q "/search/advanced" billwatch-web/src/services/searchService.ts. (cat:refactor; multifile:yes)
- [ ] [T4] billwatch-web/src/components/LegislatorSearch.vue — Refactor the legislator search request logic to use a dedicated service function that encapsulates the correct API path. VERIFY: grep -q "legislatorService" billwatch-web/src/components/LegislatorSearch.vue && grep -q "/legislators/search" billwatch-web/src/services/legislatorService.ts. (cat:refactor; multifile:yes)
- [ ] [T5] billwatch-web/src/services/api.ts — Add a unit test to verify that the API client does not double-prefix `/api` when `baseURL` already contains it. VERIFY: npx vitest run src/services/__tests__/api.test.ts 2>&1 | grep -q "passed". (cat:test; multifile:no)
...
- [ ] [T1] billwatch-web/src/pages/Admin.vue — Remove the leading `/api` prefix from the `api.get('/api/admin/users')` call to use `api.get('/admin/users')`. VERIFY: grep -n "api.get('/admin/users')" billwatch-web/src/pages/Admin.vue. (cat:vue; multifile:no)
- [ ] [T1] billwatch-web/src/pages/Admin.vue — Remove the leading `/api` prefix from the `api.get('/api/admin/statistics')` call to use `api.get('/admin/statistics')`. VERIFY: grep -n "api.get('/admin/statistics')" billwatch-web/src/pages/Admin.vue. (cat:vue; multifile:no)
- [ ] [T2] billwatch-web/src/__tests__/pages/Admin.spec.ts — Create a unit test mocking the `api` client and asserting that `fetchUsers` calls `api.get` with `/admin/users` and `fetchStats` calls `api.get` with `/admin/statistics`. VERIFY: npx vitest run billwatch-web/src/__tests__/pages/Admin.spec.ts. (cat:test; multifile:no)
- [ ] [T3] billwatch-web/src/pages/Admin.vue — Update the `fetchUsers` function to handle the response payload correctly assuming the base URL now resolves to `/api/admin/users` without double prefixing. VERIFY: npx vue-tsc --noEmit. (cat:vue; multifile:no)
- [ ] [T3] billwatch-web/src/pages/Admin.vue — Update the `fetchStats` function to handle the response payload correctly assuming the base URL now resolves to `/api/admin/statistics` without double prefixing. VERIFY: npx vue-tsc --noEmit. (cat:vue; multifile:no)
- [ ] [T4] billwatch-web/src/pages/Admin.vue — Refactor API calls to use a dedicated `adminApi` helper or ensure the shared `api` instance baseURL is correctly configured to prevent future double-prefixing errors. VERIFY: npx vitest run billwatch-web/src/__tests__/pages/Admin.spec.ts && npx vue-tsc --noEmit. (cat:refactor; multifile:no)
...
- [ ] [T1] billwatch-backend/app/models/bill_comment.py — Create SQLAlchemy model `BillComment` with columns: id, bill_id (FK), user_id (FK), content, created_at, is_deleted. VERIFY: python -c "from billwatch_backend.app.models.bill_comment import BillComment; print(BillComment.__tablename__)". (cat:python; multifile:no)
- [ ] [T2] billwatch-backend/app/schemas/bill_comment.py — Create Pydantic schemas `BillCommentCreate` and `BillCommentResponse` matching the model fields. VERIFY: python -c "from billwatch_backend.app.schemas.bill_comment import BillCommentCreate, BillCommentResponse; print(BillCommentCreate.model_fields.keys())". (cat:schema; multifile:no)
- [ ] [T3] billwatch-backend/alembic/versions/0004_add_bill_comments_table.py — Create Alembic migration to add `bill_comments` table with appropriate indexes and foreign keys. VERIFY: alembic upgrade head && psql -c "\d bill_comments". (cat:python; multifile:no)
- [ ] [T4] billwatch-backend/app/routers/bill_comments.py — Implement router with `GET /bills/{bill_id}/comments` and `POST /bills/{bill_id}/comments` endpoints, including auth dependency and basic content validation. VERIFY: pytest tests/test_bill_comments_router.py -v. (cat:endpoint; multifile:no)
- [ ] [T5] billwatch-backend/app/main.py — Register the new `bill_comments` router with the FastAPI app instance. VERIFY: python -c "from billwatch_backend.app.main import app; print(any('comments' in r.path for r in app.routes))". (cat:python; multifile:no)
...
- [ ] [T1] billwatch-backend/app/services/vote_sync_service.py — Add `upsert_votes(session, votes)` function that performs bulk upsert of vote dicts into the `Vote` model using the `(congress_id, chamber, roll_call_number)` unique constraint. VERIFY: `pytest tests/test_vote_sync_service.py::test_upsert_votes_new_and_existing -v`. (cat:python; multifile:no)
- [ ] [T2] billwatch-backend/tests/test_vote_sync_service.py — Create test file with unit tests for `upsert_votes` verifying insertion of new votes and updating of existing votes based on the unique constraint. VERIFY: `pytest tests/test_vote_sync_service.py -v`. (cat:test; multifile:no)
- [ ] [T3] billwatch-backend/app/services/vote_sync_service.py — Rewrite `sync_votes()` to instantiate `CongressAPIClient`, call `get_votes()` for both chambers, process results with `parse_vote_response()`, and call `upsert_votes()` within a DB session. VERIFY: `grep -q "CongressAPIClient" app/services/vote_sync_service.py && grep -q "parse_vote_response" app/services/vote_sync_service.py`. (cat:python; multifile:no)
- [ ] [T4] billwatch-backend/app/services/vote_sync_service.py — Add error handling and logging to `sync_votes()` to catch API exceptions from `CongressAPIClient` and log failures without crashing the scheduler job. VERIFY: `pytest tests/test_vote_sync_service.py::test_sync_votes_handles_api_error -v`. (cat:python; multifile:no)
- [ ] [T5] billwatch-backend/tests/test_vote_sync_service.py — Add integration-style test mocking `CongressAPIClient.get_votes` and `parse_vote_response` to verify `sync_votes` correctly orchestrates the fetch-parse-upsert flow. VERIFY: `pytest tests/test_vote_sync_service.py::test_sync_votes_orchestration -v`. (cat:test; multifile:no)
