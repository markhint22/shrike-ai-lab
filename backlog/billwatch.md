...
- [ ] [T1] billwatch-web/src/services/__tests__/searchService.test.ts — Create new test file that mocks `@/services/api` using `vi.mock` and imports `searchService` from `../searchService`. VERIFY: `cd billwatch-web && npx vitest run src/services/__tests__/searchService.test.ts 2>&1 | grep -q "passed"`. (cat:test; multifile:no) [feat:billwatch-20260921-add-a-direct-unit-test-for-searchservice]
- [ ] [T2] billwatch-web/src/services/__tests__/searchService.test.ts — Add test case asserting `searchService.advancedSearch` calls `api.get` with the exact string `/search/advanced` and the provided params object. VERIFY: `cd billwatch-web && npx vitest run src/services/__tests__/searchService.test.ts 2>&1 | grep -q "passed"`. (cat:test; multifile:no) [feat:billwatch-20260921-add-a-direct-unit-test-for-searchservice]
- [ ] [T3] billwatch-web/src/services/__tests__/searchService.test.ts — Add test case asserting `searchService.advancedSearch` resolves to the `data` property of the mocked API response. VERIFY: `cd billwatch-web && npx vitest run src/services/__tests__/searchService.test.ts 2>&1 | grep -q "passed"`. (cat:test; multifile:no) [feat:billwatch-20260921-add-a-direct-unit-test-for-searchservice]
- [ ] [T4] billwatch-web/src/services/searchService.ts — Verify existing implementation exports `advancedSearch` and uses `api.get('/search/advanced', { params })` without the `/api` prefix, ensuring the test target matches current code. VERIFY: `cd billwatch-web && grep -q "api.get('/search/advanced'" src/services/searchService.ts`. (cat:typescript; multifile:no) [feat:billwatch-20260921-add-a-direct-unit-test-for-searchservice]
- [ ] [T5] billwatch-web/src/services/__tests__/searchService.test.ts — Ensure the mock setup for `@/services/api` correctly intercepts the `get` method to prevent actual network calls during test execution. VERIFY: `cd billwatch-web && npx vitest run src/services/__tests__/searchService.test.ts 2>&1 | grep -q "passed"`. (cat:test; multifile:no) [feat:billwatch-20260921-add-a-direct-unit-test-for-searchservice]
