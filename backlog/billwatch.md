...
- [ ] [T1] billwatch-backend/app/routers/activity_logs.py — Add `from app.models.user import User` to the existing import block. VERIFY: `grep -q "from app.models.user import User" billwatch-backend/app/routers/activity_logs.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-activity-logs-current-user-crash]
- [ ] [T1] billwatch-backend/app/routers/activity_logs.py — In `get_activity_logs`, change the parameter type hint from `current_user: dict = Depends(get_current_user)` to `current_user: User = Depends(get_current_user)`, matching the real return type of `get_current_user` (see app/routers/auth.py). VERIFY: `grep -q "current_user: User = Depends(get_current_user)" billwatch-backend/app/routers/activity_logs.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-activity-logs-current-user-crash]
- [ ] [T1] billwatch-backend/app/routers/activity_logs.py — Replace the crashing `current_user["id"]` subscript with `current_user.id` inside the `str(...)` call in `get_activity_logs`. VERIFY: `grep -q "str(current_user.id)" billwatch-backend/app/routers/activity_logs.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-activity-logs-current-user-crash]
- [ ] [T2] billwatch-backend/tests/test_activity_logs_router.py — Replace `test_get_activity_logs`'s dict-returning `override_get_current_user` dependency override with a real authenticated flow: register a new user via `client.post("/api/auth/register", json={"email": ..., "password": ..., "display_name": ...})`, then call `client.get("/api/activity-logs/logs", headers={"Authorization": f"Bearer {resp.json()['access_token']}"})` so the test exercises the real `User` ORM instance end-to-end instead of masking the bug with a plain dict. VERIFY: `cd billwatch-backend && pytest tests/test_activity_logs_router.py -v`. (cat:backend; multifile:no) [feat:billwatch-20260922-activity-logs-current-user-crash]
- [ ] [T2] billwatch-backend/tests/test_activity_logs_router.py — Assert the rewritten `test_get_activity_logs` returns HTTP 200 (proving the former `TypeError: 'User' object is not subscriptable` no longer occurs) and that the response body is JSON-decodable, locking in the fix as a real regression guard. VERIFY: `cd billwatch-backend && pytest tests/test_activity_logs_router.py::test_get_activity_logs -v`. (cat:backend; multifile:no) [feat:billwatch-20260922-activity-logs-current-user-crash]
- [ ] [T2] billwatch-backend — Run the full activity-logs test file plus the broader router test suite once more to confirm the type-hint change doesn't break any other test that imports or calls `get_activity_logs`. VERIFY: `cd billwatch-backend && pytest tests/test_activity_logs_router.py -v`. (cat:backend; multifile:no) [feat:billwatch-20260922-activity-logs-current-user-crash]
...
- [ ] [T1] billwatch-backend/app/core/rate_limiting.py — Confirm zero production references before deleting. VERIFY: `cd billwatch-backend && ! grep -rn "rate_limiting" app/ tests/ --include="*.py" | grep -v "app/core/rate_limiting.py"`. (cat:backend; multifile:no) [feat:billwatch-20260922-delete-rate-limiting-dead-module]
- [ ] [T1] billwatch-backend/app/core/rate_limiting.py — Delete this orphaned 118-line module (Limiter/LIMIT_PUBLIC/LIMIT_SEARCH/LIMIT_AUTH/LIMIT_SENSITIVE, never imported anywhere; the real, wired-in limiter is the separate app/core/rate_limiter.py). VERIFY: `test ! -f billwatch-backend/app/core/rate_limiting.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-delete-rate-limiting-dead-module]
- [ ] [T2] billwatch-backend — Run the full backend test suite to confirm nothing depended on the deleted module (import errors would surface immediately at collection time). VERIFY: `cd billwatch-backend && pytest tests/ -x -q 2>&1 | tail -20`. (cat:backend; multifile:no) [feat:billwatch-20260922-delete-rate-limiting-dead-module]
...
- [ ] [T1] billwatch-backend/app/services/caching.py — Confirm zero production references before deleting (the real, wired-in cache is the separate app/services/caching_service.py's CachingService, used by app/routers/bills.py, app/routers/topics.py, and app/services/bill_sync_service.py). VERIFY: `cd billwatch-backend && ! grep -rn "from app.services.caching import\|services\.caching import" app/ --include="*.py"`. (cat:backend; multifile:no) [feat:billwatch-20260922-delete-caching-dead-module]
- [ ] [T1] billwatch-backend/app/services/caching.py — Delete this orphaned 58-line CacheService module. VERIFY: `test ! -f billwatch-backend/app/services/caching.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-delete-caching-dead-module]
- [ ] [T1] billwatch-backend/tests/test_cache_service.py — Delete this test file (imports only the now-deleted `app.services.caching.CacheService`). VERIFY: `test ! -f billwatch-backend/tests/test_cache_service.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-delete-caching-dead-module]
- [ ] [T1] billwatch-backend/tests/test_caching_get_or_set.py — Delete this test file (imports only the now-deleted `app.services.caching.CacheService`). VERIFY: `test ! -f billwatch-backend/tests/test_caching_get_or_set.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-delete-caching-dead-module]
- [ ] [T1] billwatch-backend/tests/test_caching.py — Delete this test file (imports only the now-deleted `app.services.caching.CacheService`; do not confuse with the separate, real tests/test_caching_service.py which must be kept). VERIFY: `test ! -f billwatch-backend/tests/test_caching.py && test -f billwatch-backend/tests/test_caching_service.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-delete-caching-dead-module]
- [ ] [T2] billwatch-backend — Run the full backend test suite to confirm the real CachingService and its callers (bills.py/topics.py/bill_sync_service.py) still pass with the orphaned module and its tests removed. VERIFY: `cd billwatch-backend && pytest tests/test_caching_service.py tests/test_bills_router.py tests/test_topics_router.py -v`. (cat:backend; multifile:yes) [feat:billwatch-20260922-delete-caching-dead-module]
...
- [ ] [T1] billwatch-backend/app/services/export.py — Confirm zero production references before deleting. VERIFY: `cd billwatch-backend && ! grep -rn "services.export import\|services\.export\b\|ExportService" app/ --include="*.py" | grep -v "app/services/export.py"`. (cat:backend; multifile:no) [feat:billwatch-20260922-finish-export-webhook-dead-code]
- [ ] [T1] billwatch-backend/app/services/export.py — Delete this orphaned 37-line ExportService stub (export_bills_pdf/export_legislator_voting_record always return empty; the real, wired-in export path is app/services/data_export_service.py, used by app/routers/data_export.py). VERIFY: `test ! -f billwatch-backend/app/services/export.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-finish-export-webhook-dead-code]
- [ ] [T1] billwatch-backend/app/services/webhook_service.py — Confirm zero production references before deleting (its sole caller, app/routers/webhooks.py, was already deleted in an earlier commit). VERIFY: `cd billwatch-backend && ! grep -rn "webhook_service\|WebhookService" app/ --include="*.py" | grep -v "app/services/webhook_service.py"`. (cat:backend; multifile:no) [feat:billwatch-20260922-finish-export-webhook-dead-code]
- [ ] [T1] billwatch-backend/app/services/webhook_service.py — Delete this orphaned 68-line WebhookService (register_webhook/get_webhooks/trigger_webhook/delete_webhook, no consumer anywhere). VERIFY: `test ! -f billwatch-backend/app/services/webhook_service.py`. (cat:backend; multifile:no) [feat:billwatch-20260922-finish-export-webhook-dead-code]
- [ ] [T2] billwatch-backend — Run the full backend test suite to confirm both deletions are safe (their dedicated test files were already removed in a prior pass, so this only needs to catch any missed indirect import). VERIFY: `cd billwatch-backend && pytest tests/ -x -q 2>&1 | tail -20`. (cat:backend; multifile:no) [feat:billwatch-20260922-finish-export-webhook-dead-code]
...
- [ ] [T1] billwatch-web/src/stores/billChat.ts — Confirm zero production references before deleting (the real, live bill-chat feature is implemented directly inside src/components/BillChatPanel.vue and src/components/BillSummaryPanel.vue, neither of which imports this store). VERIFY: `cd billwatch-web && ! grep -rln "useBillChatStore" src --include="*.vue" --include="*.ts" | grep -v "src/stores/billChat.ts" | grep -v "__tests__"`. (cat:web; multifile:no) [feat:billwatch-20260922-delete-billchat-store-dead-code]
- [ ] [T1] billwatch-web/src/stores/billChat.ts — Delete this orphaned 179-line Pinia store (fetchSummary/fetchMessages/postMessage/clearBillCache/clearAllCache, uses raw axios instead of the shared api client, but is moot since nothing calls it). VERIFY: `test ! -f billwatch-web/src/stores/billChat.ts`. (cat:web; multifile:no) [feat:billwatch-20260922-delete-billchat-store-dead-code]
