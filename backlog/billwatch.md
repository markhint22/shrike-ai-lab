...
- [ ] [T2] billwatch-web/src/components/__tests__/UnifiedBillCard.test.ts — Add test case verifying that the bill title prop is displayed in the component DOM. VERIFY: `cd billwatch-web && npx vitest run src/components/__tests__/UnifiedBillCard.test.ts -t "renders bill title"`. (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/UnifiedBillCard.test.ts — Add test case verifying that the bill status badge is rendered with the correct status text. VERIFY: `cd billwatch-web && npx vitest run src/components/__tests__/UnifiedBillCard.test.ts -t "renders status badge"`. (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/UnifiedBillCard.test.ts — Add test case verifying that clicking the card emits a 'click' event with the bill ID. VERIFY: `cd billwatch-web && npx vitest run src/components/__tests__/UnifiedBillCard.test.ts -t "emits click event"`. (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/UnifiedBillCard.test.ts — Add test case verifying that the sponsor name is displayed when provided in props. VERIFY: `cd billwatch-web && npx vitest run src/components/__tests__/UnifiedBillCard.test.ts -t "renders sponsor name"`. (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/UnifiedBillCard.test.ts — Add test case verifying that the component handles missing optional props gracefully without crashing. VERIFY: `cd billwatch-web && npx vitest run src/components/__tests__/UnifiedBillCard.test.ts -t "handles missing props"`. (cat:test; multifile:no)
...
- [ ] [T1] billwatch-web/src/components/__tests__/BillChatPanel.test.ts — Create test file with Vitest setup and import BillChatPanel.vue VERIFY: npx vitest run src/components/__tests__/BillChatPanel.test.ts --passWithNoTests (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/BillChatPanel.test.ts — Add test case verifying component renders without error and displays initial empty state VERIFY: npx vitest run src/components/__tests__/BillChatPanel.test.ts -t "renders" (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/BillChatPanel.test.ts — Add test case verifying message list renders when props.messages contains data VERIFY: npx vitest run src/components/__tests__/BillChatPanel.test.ts -t "message list" (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/BillChatPanel.test.ts — Add test case verifying input field exists and is enabled when not loading VERIFY: npx vitest run src/components/__tests__/BillChatPanel.test.ts -t "input field" (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/BillChatPanel.test.ts — Add test case verifying send button emits 'send' event with input value on click VERIFY: npx vitest run src/components/__tests__/BillChatPanel.test.ts -t "send event" (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/BillChatPanel.test.ts — Add test case verifying loading state disables input and shows spinner VERIFY: npx vitest run src/components/__tests__/BillChatPanel.test.ts -t "loading state" (cat:test; multifile:no)
- [ ] [T2] billwatch-web/src/components/__tests__/BillChatPanel.test.ts — Add test case verifying error message displays when props.error is set VERIFY: npx vitest run src/components/__tests__/BillChatPanel.test.ts -t "error display" (cat:test; multifile:no)
...
- [ ] [T2] billwatch-backend/app/schemas/auth.py — Add `dark_mode: bool = False` and `language: str = "en"` fields to `UserResponse` class. VERIFY: `python -c "from app.schemas.auth import UserResponse; assert hasattr(UserResponse, 'dark_mode') and hasattr(UserResponse, 'language')"`. (cat:schema; multifile:no)
- [ ] [T2] billwatch-backend/app/schemas/auth.py — Create `SettingsUpdate` Pydantic model with optional `dark_mode: bool | None = None` and `language: str | None = None` fields. VERIFY: `python -c "from app.schemas.auth import SettingsUpdate; s=SettingsUpdate(dark_mode=True); assert s.dark_mode is True"`. (cat:schema; multifile:no)
- [ ] [T3] billwatch-backend/app/routers/auth.py — Add `PATCH /me/settings` endpoint that accepts `SettingsUpdate`, updates the user's `dark_mode` and `language` attributes, commits to DB, and returns the updated `UserResponse`. VERIFY: `grep -q "def update_user_settings" billwatch-backend/app/routers/auth.py && grep -q "@router.patch(\"/me/settings\")" billwatch-backend/app/routers/auth.py`. (cat:endpoint; multifile:no)
- [ ] [T1] billwatch-backend/tests/test_auth_settings.py — Create test file with a unit test mocking the DB session to verify `update_user_settings` correctly sets `dark_mode=True` and `language="fr"` on the user object. VERIFY: `pytest billwatch-backend/tests/test_auth_settings.py::test_update_settings_success -v`. (cat:test; multifile:no)
- [ ] [T1] billwatch-backend/tests/test_auth_settings.py — Add unit test verifying that `SettingsUpdate` with no fields provided does not alter existing user settings values. VERIFY: `pytest billwatch-backend/tests/test_auth_settings.py::test_update_settings_no_change -v`. (cat:test; multifile:no)
- [ ] [T1] billwatch-backend/tests/test_auth_settings.py — Add unit test verifying that the endpoint returns 401 if no user is authenticated (mocking dependency failure). VERIFY: `pytest billwatch-backend/tests/test_auth_settings.py::test_update_settings_unauthorized -v`. (cat:test; multifile:no)
- [ ] [T2] billwatch-backend/app/schemas/auth.py — Ensure `UserResponse` model configuration allows extra fields or explicitly includes the new columns to prevent serialization errors. VERIFY: `python -c "from app.schemas.auth import UserResponse; u=UserResponse(id=1, email='a@b.c', dark_mode=True, language='en'); assert u.dark_mode"`. (cat:schema; multifile:no)
