...
- [ ] [T4] scripts/grid/manhattan.gd — Delete the file `scripts/grid/manhattan.gd` as it is a duplicate of `manhattan_distance.gd` and has no external references. VERIFY: Run `grep -r "class_name Manhattan" --include="*.gd" . | grep -v "ManhattanDistance"` and confirm no output; run `ls scripts/grid/manhattan.gd` and confirm "No such file or directory". (cat:refactor; multifile:yes) [feat:xlite-20260920-grid-dedupe-two-identical-manhattan-dist]
- [ ] [T4] tests/test_manhattan.gd — Delete the test file `tests/test_manhattan.gd` as it tests the deleted duplicate class. VERIFY: Run `ls tests/test_manhattan.gd` and confirm "No such file or directory"; run `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests` to ensure no test suite errors due to missing files. (cat:test; multifile:yes) [feat:xlite-20260920-grid-dedupe-two-identical-manhattan-dist]
