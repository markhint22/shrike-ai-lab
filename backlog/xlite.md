...
- [ ] [T5] tests — Run the full GUT suite once to confirm `total_scrap_for_tier` caused zero regressions elsewhere. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-salvagevalue-total-for-tier]
...
- [ ] [T1] scripts/battle/execute_threshold.gd — Add `static func percent_above_threshold(hp: int, max_hp: int, threshold_percent: int) -> int`: return 0 if `max_hp <= 0`; otherwise let `hp_percent = clampi(hp * 100 / max_hp, 0, 100)`, return `maxi(hp_percent - threshold_percent, 0)`. VERIFY: `grep -q "static func percent_above_threshold" scripts/battle/execute_threshold.gd`. (cat:godot; multifile:no) [feat:xlite-20260922-executethreshold-percent-above]
- [ ] [T2] tests/test_execute_threshold_percent_above.gd — Create new GUT test file (`extends GutTest`) with `test_above_threshold` asserting `ExecuteThreshold.percent_above_threshold(50, 100, 30) == 20`. VERIFY: `grep -q "extends GutTest" tests/test_execute_threshold_percent_above.gd`. (cat:test; multifile:no) [feat:xlite-20260922-executethreshold-percent-above]
- [ ] [T2] tests/test_execute_threshold_percent_above.gd — Add `test_below_threshold_floors_at_zero` asserting `ExecuteThreshold.percent_above_threshold(20, 100, 30) == 0`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_execute_threshold_percent_above.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-executethreshold-percent-above]
- [ ] [T2] tests/test_execute_threshold_percent_above.gd — Add `test_full_hp_zero_threshold` asserting `ExecuteThreshold.percent_above_threshold(100, 100, 0) == 100`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_execute_threshold_percent_above.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-executethreshold-percent-above]
- [ ] [T3] tests/test_execute_threshold_percent_above.gd — Add `test_zero_max_hp_guard` asserting `ExecuteThreshold.percent_above_threshold(10, 0, 30) == 0`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_execute_threshold_percent_above.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-executethreshold-percent-above]
- [ ] [T3] tests/test_execute_threshold_percent_above.gd — Add `test_exactly_at_threshold` asserting `ExecuteThreshold.percent_above_threshold(30, 100, 30) == 0`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_execute_threshold_percent_above.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-executethreshold-percent-above]
- [ ] [T4] scripts/battle/execute_threshold.gd — Add a doc comment above `percent_above_threshold` explaining it feeds an HP-bar overlay showing how close a target is above the execute line. VERIFY: `grep -q "HP-bar" scripts/battle/execute_threshold.gd`. (cat:docs; multifile:no) [feat:xlite-20260922-executethreshold-percent-above]
- [ ] [T5] tests — Run the full GUT suite once to confirm `percent_above_threshold` caused zero regressions elsewhere. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-executethreshold-percent-above]

# --- Claude-decomposed from roadmap [2026-09-22 deep pass]: HeightDamage levels-needed-for-bonus — scripts/battle/height_damage.gd ---
- [ ] [T1] scripts/battle/height_damage.gd — Add `static func levels_needed_for_bonus(target_percent: int, per_level: int) -> int` (inverse of `bonus_percent`): return -1 if `per_level <= 0`; return 0 if `target_percent <= 0`; otherwise `ceili(float(target_percent) / float(per_level))`. VERIFY: `grep -q "static func levels_needed_for_bonus" scripts/battle/height_damage.gd`. (cat:godot; multifile:no) [feat:xlite-20260922-heightdamage-levels-needed]
- [ ] [T2] tests/test_height_damage_levels_needed.gd — Create new GUT test file (`extends GutTest`) with `test_exact_roundtrip` asserting `HeightDamage.levels_needed_for_bonus(30, 10) == 3` and `HeightDamage.bonus_percent(3, 0, 10) == 30`. VERIFY: `grep -q "extends GutTest" tests/test_height_damage_levels_needed.gd`. (cat:test; multifile:no) [feat:xlite-20260922-heightdamage-levels-needed]
- [ ] [T2] tests/test_height_damage_levels_needed.gd — Add `test_rounds_up` asserting `HeightDamage.levels_needed_for_bonus(31, 10) == 4`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_height_damage_levels_needed.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-heightdamage-levels-needed]
- [ ] [T2] tests/test_height_damage_levels_needed.gd — Add `test_zero_target_needs_no_levels` asserting `HeightDamage.levels_needed_for_bonus(0, 10) == 0`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_height_damage_levels_needed.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-heightdamage-levels-needed]
- [ ] [T3] tests/test_height_damage_levels_needed.gd — Add `test_zero_per_level_is_unreachable` asserting `HeightDamage.levels_needed_for_bonus(30, 0) == -1`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_height_damage_levels_needed.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-heightdamage-levels-needed]
- [ ] [T3] tests/test_height_damage_levels_needed.gd — Add `test_negative_target_needs_no_levels` asserting `HeightDamage.levels_needed_for_bonus(-5, 10) == 0`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_height_damage_levels_needed.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-heightdamage-levels-needed]
- [ ] [T4] scripts/battle/height_damage.gd — Add a doc comment above `levels_needed_for_bonus` explaining it is the inverse of `bonus_percent`, for a positioning-hint overlay. VERIFY: `grep -q "inverse" scripts/battle/height_damage.gd`. (cat:docs; multifile:no) [feat:xlite-20260922-heightdamage-levels-needed]
- [ ] [T5] tests — Run the full GUT suite once to confirm `levels_needed_for_bonus` caused zero regressions elsewhere. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-heightdamage-levels-needed]

# --- Claude-decomposed from roadmap [2026-09-22 deep pass]: StatGrowth levels-needed-for-stat — scripts/roster/stat_growth.gd ---
- [ ] [T1] scripts/roster/stat_growth.gd — Add `static func levels_needed_for_stat(base: int, target_stat: int, per_level: int) -> int` (inverse of `stat_at_level`): return 1 if `target_stat <= base`; return -1 if `per_level <= 0`; otherwise `1 + ceili(float(target_stat - base) / float(per_level))`. VERIFY: `grep -q "static func levels_needed_for_stat" scripts/roster/stat_growth.gd`. (cat:godot; multifile:no) [feat:xlite-20260922-statgrowth-levels-needed]
- [ ] [T2] tests/test_stat_growth_levels_needed.gd — Create new GUT test file (`extends GutTest`) with `test_exact_roundtrip` asserting `StatGrowth.levels_needed_for_stat(10, 25, 5) == 4` and `StatGrowth.stat_at_level(10, 4, 5) == 25`. VERIFY: `grep -q "extends GutTest" tests/test_stat_growth_levels_needed.gd`. (cat:test; multifile:no) [feat:xlite-20260922-statgrowth-levels-needed]
- [ ] [T2] tests/test_stat_growth_levels_needed.gd — Add `test_rounds_up` asserting `StatGrowth.levels_needed_for_stat(10, 26, 5) == 5`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_stat_growth_levels_needed.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-statgrowth-levels-needed]
- [ ] [T2] tests/test_stat_growth_levels_needed.gd — Add `test_already_at_target_needs_level_one` asserting `StatGrowth.levels_needed_for_stat(10, 10, 5) == 1` and `StatGrowth.levels_needed_for_stat(10, 5, 5) == 1`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_stat_growth_levels_needed.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-statgrowth-levels-needed]
- [ ] [T3] tests/test_stat_growth_levels_needed.gd — Add `test_zero_per_level_is_unreachable` asserting `StatGrowth.levels_needed_for_stat(10, 20, 0) == -1`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_stat_growth_levels_needed.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-statgrowth-levels-needed]
- [ ] [T3] tests/test_stat_growth_levels_needed.gd — Add `test_negative_per_level_is_unreachable` asserting `StatGrowth.levels_needed_for_stat(10, 20, -5) == -1`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_stat_growth_levels_needed.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-statgrowth-levels-needed]
- [ ] [T4] scripts/roster/stat_growth.gd — Add a doc comment above `levels_needed_for_stat` explaining it is the inverse of `stat_at_level`, for a roster-planning "what level gets me to X" tooltip. VERIFY: `grep -q "inverse" scripts/roster/stat_growth.gd`. (cat:docs; multifile:no) [feat:xlite-20260922-statgrowth-levels-needed]
- [ ] [T5] tests — Run the full GUT suite once to confirm `levels_needed_for_stat` caused zero regressions elsewhere. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-statgrowth-levels-needed]

# --- Claude-decomposed from roadmap [2026-09-22 deep pass]: ReloadCost ap-after-reload — scripts/battle/reload_cost.gd ---
- [ ] [T1] scripts/battle/reload_cost.gd — Add `static func ap_after_reload(ap: int, missing: int, per_clip: int) -> int` implemented as `return maxi(ap - ap_to_reload(missing, per_clip), 0)`, reusing `ap_to_reload` rather than recomputing it. VERIFY: `grep -q "static func ap_after_reload" scripts/battle/reload_cost.gd`. (cat:godot; multifile:no) [feat:xlite-20260922-reloadcost-ap-after]
- [ ] [T2] tests/test_reload_cost_ap_after.gd — Create new GUT test file (`extends GutTest`) with `test_affordable_reload` asserting `ReloadCost.ap_after_reload(5, 3, 2) == 3`. VERIFY: `grep -q "extends GutTest" tests/test_reload_cost_ap_after.gd`. (cat:test; multifile:no) [feat:xlite-20260922-reloadcost-ap-after]
- [ ] [T2] tests/test_reload_cost_ap_after.gd — Add `test_insufficient_ap_floors_at_zero` asserting `ReloadCost.ap_after_reload(1, 3, 2) == 0`. VERIFY: `godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://tests/test_reload_cost_ap_after.gd -gexit`. (cat:test; multifile:no) [feat:xlite-20260922-reloadcost-ap-after]
