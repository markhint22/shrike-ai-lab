...
STUB
    ...
    if [ "${OVN_INLINE_STAGE:-1}" = 1 ] && [ -f "OVERNIGHT_PROGRESS.md" ] \
      ...
      # wrapper script — whose `trap cleanup EXIT` immediately `git worktree remove --force`d the
      # temp worktree — while the still-running pytest full-verify subprocess (a grandchild, never
      # signaled) kept executing against a directory that had just been deleted out from under it,
      # crashing with FileNotFoundError near the end of an otherwise-passing run. Confirmed live via
      # state/stage_runs/iptv_apps-*.verify.log: a 98%-through pytest run died on
      ...
      if grep -qE 'no doable T3\+ item found|another stage runner holds the lock|decompose produced no steps' "$_STAGE_OUT"; then
        rm -f "$_STAGE_OUT"
        # 2026-09-25 FIX: "decompose produced no steps" (decompose_failed) now falls through
        # here too - a repo whose T3-5 backlog is all too-hard-to-decompose items (xlite: 5 T5
        # items, 12h, 0 landed of any tier) had every cycle consumed by this guaranteed-
        # unproductive attempt. See ovn_stage_runner.sh's 2026-09-15 comment for the
        # "still-open gap" this closes.
...
If this task is about DELETING a file: that is NOT a tooling blocker and NOT a decision - deleting
a file is fully supported here via a commit-message trailer (DELETE: <path>), never by editing/
emptying the file or by a diff/patch hunk. VERDICT should be PROCEED for a pure file-deletion item
just like any other change (2026-09-22: found live across gitlark/iptv_apps - scout was reasoning
'the patch tool can't delete files' and answering BLOCKED/NEEDS-DECISION for exactly this reason,
even though the implement step already knows the working DELETE: mechanism and would have landed
it - the scout just never learned that a solution exists here).
