#!/usr/bin/env bash
# GPU window: Qwen-Image 2.1 wasteland floor re-fix (calm/low-contrast redesign,
# 5 variants, text2img). Trap-safe: always clears art_window.hold + restarts
# inference on exit. Does NOT touch queue.sh pause state (orchestrating session
# owns that). Modeled directly on qwen21_terrain_eval_window.sh.
set -uo pipefail
Q=/home/mhintermeister/overnight-queue
STATE=$Q/state
OUT=$Q/reports/qwen21-floor-fix.out
VENV=/run/media/mhintermeister/secondary_drive1/comfy/models_eval/.venv-qwen21
SCRIPT=/run/media/mhintermeister/secondary_drive1/comfy/models_eval/gen_qwen21_floor_fix.py
C=shrike-llama-dflash-35b

mkdir -p "$Q/reports"
exec 9>/tmp/qwen21_floor_fix.lock
...
restore(){
  ...
  log "=== QWEN21 FLOOR FIX WINDOW DONE ==="
}
trap restore EXIT
...
log "=== GENERATING floor fix (5 variants) ==="
