...
# --- 2. actionable alerts (NOT the healthy gate activity) ------------------
# alerts.log is dominated by build-gate / no-new-red reverts and red-green flags,
# which are the safety gates WORKING (bad code caught, nothing broke) - not things
# that need you. Surface ONLY genuinely-actionable alerts, and archive the log so
# it doesn't re-accumulate forever.
if [ -s "$STATE/alerts.log" ]; then
  actionable="$(grep -vE 'build-gate reverted|reverted a commit that left tests red|red-green:' "$STATE/alerts.log" 2>/dev/null | grep -c . 2>/dev/null)"
  if [ "${actionable:-0}" -gt 0 ]; then
    newest="$(grep -vE 'build-gate reverted|reverted a commit that left tests red|red-green:' "$STATE/alerts.log" | tail -1 | sed 's/^[^|]*| //')"
    findings+=("${actionable} alert(s) need a look — newest: ${newest}")
  fi
  # archive so the same alerts aren't re-counted next run (real conditions like an
  # auto-disabled task are re-detected from their own state each run).
  cat "$STATE/alerts.log" >> "$STATE/alerts.archive.log" 2>/dev/null || true
  : > "$STATE/alerts.log"
fi
...
for f in "$STATE"/branch_hygiene_review_* ; do
  [ -e "$f" ] || continue
  findings+=("branch-hygiene flagged: $(basename "$f" | sed 's/^branch_hygiene_review_//') — feature->develop gate not green.")
done

# --- 6. overnight/feature drift ahead of DEVELOP (unlanded work) ----------
# Measure vs develop, NOT main: the fleet lands feature->develop continuously and
# develop->main is promoted only once a day, so feature is EXPECTED to run far ahead
# of main between promotes (that is not drift). feature far ahead of *develop* is the
# real "hygiene isn't landing" signal.
if [ -d "$REPOS" ]; then
  for r in "$REPOS"/*/ ; do
    [ -d "$r/.git" ] || continue
    name="$(basename "$r")"
...
# --- deterministic WORK summary (what the fleet actually DID — throughput, by tier) --------
# Added 2026-09-07: the push used to fire ONLY on problems ("N items need attention"), so a normal
# working night looked silent and you couldn't tell what landed. This surfaces real throughput.
WORK_SUMMARY="$(cd "$DIR" 2>/dev/null && python3 work_summary.py 24 2>/dev/null)"
...
REPORT_FILE="$REPORT_DIR/supervisor-$(date +%Y%m%d-%H%M%S).md"
{
  ...
  echo ""
  [ -n "$WORK_SUMMARY" ] && { echo "## Work (last 24h)"; echo "$WORK_SUMMARY"; echo ""; }
...
# --- phone push: problems fire immediately; throughput fires ONCE/day even when all-clear -----
